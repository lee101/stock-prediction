...
static int should_drawdown_profit_early_exit(
    const TradingEnv* env, float equity_after,
    float* out_progress, float* out_total_return
) {
    const AgentState* ag = &env->agent;
    ...
    if (!env->enable_drawdown_profit_early_exit) return 0;
    if (total_steps < 2) return 0;
    if (total_steps < min_steps) return 0;
    if (!isfinite(ag->initial_equity) || ag->initial_equity <= 0.0f) return 0;

    float progress = (float)(ag->step + 1) / (float)total_steps;
    if (progress > 1.0f) progress = 1.0f;
    if (out_progress) *out_progress = progress;
    if (progress < progress_fraction) return 0;
...
static int resolve_limit_fill_price(const TradingEnv* env, const MarketData* md, int t, int sym, float target_price, int is_buy, float* fill_price) {
    ...
    if (target_price < low || target_price > high) return 0;
    float slippage = env->fill_slippage_bps / 10000.0f;
    if (is_buy) {
        *fill_price = target_price * (1.0f + slippage);
        if (*fill_price > high) return 0;
    } else {
        *fill_price = target_price * (1.0f - slippage);
        if (*fill_price < low) return 0;
    }
    return 1;
}
...
static float action_level_offset_bps(const TradingEnv* env, int level_idx) {
    ...
    float frac = (float)level_idx / (float)(bins - 1);
    return (2.0f * frac - 1.0f) * max_bps;
}
...
static void open_long(TradingEnv* env, int t, int sym, float allocation_pct, float level_offset_bps) {
    ...
    if (target_price <= 0.0f || !resolve_limit_fill_price(env, md, t, sym, target_price, 1, &fill_price))
        return;
...
static void open_short(TradingEnv* env, int t, int sym, float allocation_pct, float level_offset_bps) {
    ...
    float target_price = ref_price * (1.0f + level_offset_bps / 10000.0f);
    float fill_price = 0.0f;
    if (target_price <= 0.0f || !resolve_limit_fill_price(env, md, t, sym, target_price, 0, &fill_price))
        return;
...
__attribute__((hot)) void c_step(TradingEnv* env) {
    ...
    if (md->tradable != NULL) {
        any_tradable = 0;
        for (int i = 0; i < S; i++) {
            if (is_tradable(md, t, i)) { any_tradable = 1; break; }
        }
    }
    ...
    ag->sum_ret += ret;
    ag->ret_count++;
    if (ret < 0.0f) ag->sum_neg_sq += ret * ret;

    if (equity_after > ag->peak_equity) ag->peak_equity = equity_after;
    float current_drawdown = 0.0f;
    if (ag->peak_equity > 0.0f) {
        current_drawdown = (ag->peak_equity - equity_after) / ag->peak_equity;
        if (current_drawdown > ag->max_drawdown) ag->max_drawdown = current_drawdown;
    }
    ...
    if (ag->position_sym < 0 && any_tradable) reward -= env->cash_penalty;
    ...
    if (env->smooth_downside_penalty > 0.0f) {
        ...
        float x = -ret / temp;
        float softplus_x;
        if (x > 20.0f) softplus_x = x;
        else if (x < -20.0f) softplus_x = expf(x);
        else softplus_x = log1pf(expf(x));
    ...
    if (should_drawdown_profit_early_exit(env, equity_after, &early_exit_progress, &early_exit_total_return))
        early_exit = 1;
    ...
    int done = (ag->step >= env->max_steps) ||
               ...
               early_exit ||
               (equity_after < INITIAL_CASH * 0.01f);

    if (UNLIKELY(done)) {
        if (ag->position_sym >= 0) close_position(env, t_new);
        ...
        env->log.win_rate += (ag->num_trades > 0)
            ? (float)ag->winning_trades / ag->num_trades : 0.0f;
        env->log.avg_hold_hours += (ag->num_trades > 0)
            ? (float)ag->step / ag->num_trades : 0.0f;
        env->log.n += 1.0f;
