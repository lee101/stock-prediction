...
def per_pair_pnl(trades):
    """Compute per-pair PnL breakdown from trade list."""
...
def run_experiment(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=parallel) as pool:
        ...
        for f in concurrent.futures.as_completed(futs):
            ...
            if done % max(1, total // 10) == 0:
                ...
                print(f"    [{variant}/{model}] {done}/{total} ({rate:.1f}/s)")

    ar, br = [], []
    stats = {"long": 0, "hold": 0, "short_filtered": 0}
    for s, b, plan, _rs in results:
        ...
        if plan.direction == "short":
            ...
            plan = TradePlan("hold", 0, 0, 0, "invalid")  # noqa: PLW2901
    ...
    # Per-pair breakdown
    pp = per_pair_pnl(trades)

    print(f"\n  [{variant}/{model}] RESULTS:")
...
def main():
    ...
    p.add_argument("--symbols", nargs="+", default=["BTCUSD", "ETHUSD", "SOLUSD", "DOGEUSD", "SUIUSD", "AAVEUSD"])
    p.add_argument("--days", type=int, default=7)
    p.add_argument("--gemini-model", default="gemini-3.1-flash-lite-preview")
    ...
    header = f"{'Pair':<10}" + "".join(f"{v:<25}" for v in results)
    print(header)
    print("-" * len(header))
    for sym in all_syms:
        parts = [f"{sym:<10}"]
        for _v, r in results.items():
            ...
            parts.append(f"${pnl:>+8.2f} ({ent:>2d}t)          ")
        print("".join(parts))
    ...
    out = RESULTS_DIR / "prompt_experiment_v3.json"
