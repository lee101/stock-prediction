...
def main():
    ...
    for symbol in symbols:
        ...
        results = []
        for i, (buy_off, sell_off, intensity) in enumerate(itertools.product(BUY_OFFSETS, SELL_OFFSETS, INTENSITIES)):
        ...
        for r in results[: args.top_k]:
            print(
                f"  {r['buy_off']:+8.4f} {r['sell_off']:+8.4f} {r['intensity']:6.2f} {r['val_sortino']:+10.2f} {r['val_return']:+10.4f}"
            )
        ...
        for r in results[:3]:
            ...
            print(
                f"    buy={r['buy_off']:+.4f} sell={r['sell_off']:+.4f} int={r['intensity']:.2f} "
        ...
        print(
            f"  WINNER: buy={winner['buy_off']:+.4f} sell={winner['sell_off']:+.4f} "
    ...
    for sym, res in all_results.items():
        w = res["winner"]
        print(
            f"{sym:<10} {w['buy_off']:+8.4f} {w['sell_off']:+8.4f} {w['intensity']:6.2f} "
