...
Last synced: **2026-04-22 15:10 UTC — hourly monitor. All Phase 1 green: XGB RUNNING (1d 16h uptime, pid=3315287), singleton lock=xgb_live_trader matches, 0 stock positions (only crypto dust < $0.01), today's session fired at 13:30 UTC with top-1 score=0.5542 on 668 candidates (3-session sequence 0.7036→0.6005→0.5542 = healthy R9b low-conviction regime). RL path confirmed DOWN (port 8050 closed, both supervisor units STOPPED). Equity $28,678.98 unchanged (hold-cash regime). Harvested prior-hour sweeps: (1) 15-seed `oos2024_ensemble_gpu_fresh` 126-cell sweep — LIVE config cells (lev=1.0/1.5/2.0 × ms=0.85 ht=Y tn=1 vol=0.12 dolvol=50M) all 0 trades / 0/63 neg / +0.00%/mo across BOTH deploy and stress36x regimes — confirms LIVE hold-cash mirrors OOS exactly; (2) 15-seed `retrain_through_2026_02_28_ensemble` 72-cell heldout sweep — ms=0.85 cells also 0 trades, lev=2.0 ms=0.60 tn=2 deploy = +9.75%/mo but 2/5 neg (40%) — fails neg-frac gate, refutes 5-seed +23.61% tail-bias claim. Bonferroni-scale null confirmed on both TRUE-OOS and heldout. NEW Phase 3 fire-and-forget launched PID=1322020 → `scripts/xgb_short_history_2024start_15seed_train_then_sweep.sh`: novel direction = train 15-seed XGB on 2024-01-01 → 2026-02-28 ONLY (26 months, vs standard 6+ years from 2020), test if regime-shift bias is responsible for full-history exhaustion. Output → `analysis/xgbnew_daily/short_history_2024start_through_0228_15seed/` + `sweep_20260422_short_history_2024start_15seed/`; harvest next hour.**
