...
Last synced: **2026-04-22 16:10 UTC — hourly monitor. All Phase 1 green: XGB RUNNING (1d 17h uptime, pid=3315287), singleton lock=xgb_live_trader matches, 0 stock positions (only crypto dust < $0.01), today's session fired at 13:30 UTC with top-1 score=0.5542 on 668 candidates (3-session sequence 0.7036→0.6005→0.5542 = healthy R9b low-conviction regime). RL path confirmed DOWN (port 8050 closed, both supervisor units STOPPED). Equity $28,678.98 unchanged (hold-cash regime). Last fills 2026-04-18 DOGE buy/sell (crypto, not XGB). HARVESTED prior-hour short-history run: 15-seed XGB trained 2024-01-01→2026-02-28 on 846 symbols, 504-cell sweep on 2026-03-01→04-20 heldout — 0/504 cells pass deploy gate (med≥27% p10≥0 neg_frac≤20% dd≤7.18). Best deploy-fee cell lev=2 ms=0.55 tn=2 vol=0.10 = +16.54%/mo but p10=−34.70, 6/15 neg (40%), dd=16.45%; stress36x 0 pos-median cells. Short-history is NOT the lever — same hold-cash profile at ms=0.85 as LIVE. Adds another GBDT null to the exhaustion ledger. NEW Phase 3 fire-and-forget PID=2005186 → `scripts/launch_late_subfold_shorthist.sh`: re-eval same 15-seed ensemble on LATE SUBFOLD ONLY (2026-03-21→04-20) to test if the +16.54%/mo full-heldout cell is a recovery-rally artifact (like MuonMLP retrain-through-0228 was). Output → `analysis/xgbnew_daily/sweep_20260422_short_history_2024start_15seed_lateonly/`; harvest next hour.**
