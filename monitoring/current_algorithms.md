...
Last synced: **2026-04-22 18:00 UTC — hourly monitor. Phase 1 GREEN: XGB RUNNING (1d 19h uptime, pid=3315287), singleton lock=xgb_live_trader matches pid, 0 stock pos (correct hold-cash regime), equity $28,678.98 unchanged. Today's 2026-04-22 session fired with top-1 score=0.5542 on 668 candidates — 3-session sequence 0.7036→0.6005→0.5542 across distinct syms = healthy R9b low-conviction regime (scores vary, not stale). RL path DOWN. HARVESTED leverage-curve 15-seed freshens sweep (pid=2729830, 55min): `analysis/xgbnew_ensemble_sweep/leverage_curve_freshens_20260422_1710/sweep_20260422_180152.json` — 98 cells (49 deploy × 2 fee regimes). Result **0/98 pass deploy gate** (p10>0 + 0neg). At ms≥0.75 on `oos2024_ensemble_gpu_fresh`: **0 trades fire across 42 windows** (structural inertia — matches known crash-regime score cap). At ms∈{0.55,0.60,0.65,0.70} trades fire but every cell has neg p10 ranging −34 to −89% and 7-27 neg windows per 42. Deployed lev=2.0 ms=0.85 = 0 trades = 0/42 neg = hold-cash. Confirms `project_xgb_score_calibration_collapse_2026_04_21.md` + 15-seed bonferroni REFUTES every ms/lev combo at deploy fees on clean true-OOS data. **Hold-cash via inert ms=0.85 gate remains the correct live behavior.** No candidate. No new fire-and-forget queued — the big ticket item was harvested this hour.**
