"""Agent scheduler for Binance PufferLib RL autoresearch.

Fork of src/autoresearch_binance/agent_scheduler.py adapted for PufferLib PPO.
"""
...
DEFAULT_ANALYSIS_DIR = Path("analysis/autoresearch_binance_agent")
DEFAULT_EXPERIMENT_BUNDLE_ROOT = Path("experiments/autoresearch_binance_agent")
DEFAULT_EXPERIMENT_CODE_ROOT = Path("src/autoresearch_binance/experiments")
DEFAULT_PYTHON = Path(".venv/bin/python")
DEFAULT_BACKENDS = ("codex",)
DEFAULT_FREQUENCIES = ("binance",)
...
TRAIN_PY_RELATIVE = "src/autoresearch_binance/train.py"
MODULE_NAME = "autoresearch_binance"
...
def build_turn_prompt(
    *,
    ...
    train_log = run_dir / "train_binance.log"
    result_json = run_dir / "agent_result.json"
    experiment_code_root = repo_root / DEFAULT_EXPERIMENT_CODE_ROOT
    prompt = f"""
You are running one bounded optimization turn for the Binance PufferLib RL autoresearch framework.
...
The model is an MLP policy (TradingPolicy) trained with PPO on a C market simulator
via PufferLib. It trades 4 crypto symbols (BTC, ETH, DOGE, AAVE) with
discrete actions (9 total: flat + 4 long + 4 short). Evaluation runs on 7d/14d/30d holdout windows.

IMPORTANT: The training uses the C binding directly (not TradingEnv gymnasium wrapper).
See how vec_init/vec_step/vec_reset/vec_close are used in train.py.
PYTHONPATH must include src/ for the module to import correctly.

Hard rules:
- Keep changes deterministic and replayable.
- Isolate multi-file experiments under `src/autoresearch_binance/experiments/<slug>/`.
- Keep `src/autoresearch_binance/train.py` as a small dispatcher.
- Do NOT edit `src/autoresearch_binance/prepare.py`.
- Do NOT fake the benchmark. Run one real training run with the fixed harness.
- Use the exact interpreter shown above.

Read first:
- `src/autoresearch_binance/program.md`
- `src/autoresearch_binance/prepare.py`
- `src/autoresearch_binance/train.py`
- `pufferlib_market/train.py` (reference PPO + policy architectures)
- `pufferlib_market/environment.py` (C env wrapper, TradingEnvConfig)
- `pufferlib_market/evaluate.py` (holdout evaluation)

Recent history:
{os.linesep.join(_history_summary_lines(recent_history))}

Required training command:
`PYTHONPATH={repo_root}/src:$PYTHONPATH {python_path} -m {MODULE_NAME}.train > {train_log} 2>&1`
    ...
    *,
    ...
    _write_text(
        ...
        "\n".join([
            ...
            "set -euo pipefail",
            f'cd "{repo_root}"',
            f'PYTHONPATH="{repo_root}/src:$PYTHONPATH" "{python_path}" -m {MODULE_NAME}.train > "{run_dir / "train_binance.log"}" 2>&1',
            "",
        ]),
    )
    ...
    if isinstance(raw_train_log, str) and raw_train_log.strip():
        ...
        default_path = run_dir / "train_binance.log"
    ...
    _copy_if_exists(train_log_path, experiment_dir / "train_binance.log")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Schedule optimization turns for autoresearch_binance.")
