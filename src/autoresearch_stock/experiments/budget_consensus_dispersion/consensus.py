...
def _consensus_adjusted_probs(
    probs: np.ndarray,
    *,
    broad_consensus_power: float,
    selective_reallocation: float,
    ...
    group_probs = values.mean(axis=0).astype(np.float32, copy=False)
    if len(values) == 1:
        return group_probs, 1.0

    pairwise_distances = np.abs(values[:, None, :] - values[None, :, :]).sum(axis=2) / 2.0
    upper = np.triu_indices(len(values), k=1)
    mean_pairwise_distance = float(pairwise_distances[upper].mean()) if upper[0].size > 0 else 0.0
    consensus = float(np.clip(1.0 - mean_pairwise_distance, 0.0, 1.0))

    adjusted = group_probs.copy()
    broad_mass = float(adjusted[2])
    retained_broad = broad_mass * (consensus ** max(float(broad_consensus_power), 1e-6))
    shifted_broad = max(broad_mass - retained_broad, 0.0)
    adjusted[2] = retained_broad
    selective_share = float(np.clip(selective_reallocation, 0.0, 1.0))
    adjusted[1] += shifted_broad * selective_share
    adjusted[0] += shifted_broad * (1.0 - selective_share)
    total = float(adjusted.sum())
    if total <= 0.0:
        return group_probs, consensus
    return (adjusted / total).astype(np.float32, copy=False), consensus


def apply_budget_consensus_dispersion_gate(
    ...
    *,
    ...
    broad_consensus_power: float,
    selective_reallocation: float,
    consensus_gap_floor: float,
    consensus_fractional_floor: float,
) -> np.ndarray:
    ...
    gap_floor = float(np.clip(consensus_gap_floor, 0.0, 1.0))
    fractional_floor = float(np.clip(consensus_fractional_floor, 0.0, 1.0))
    ...
    for _, group_index in timestamps.groupby(timestamps, sort=False).groups.items():
        ...
        symbol_probs = F.softmax(torch.from_numpy(budget_values[group_indices]), dim=-1).cpu().numpy()
        adjusted_probs, consensus = _consensus_adjusted_probs(
            symbol_probs,
            broad_consensus_power=float(broad_consensus_power),
            selective_reallocation=float(selective_reallocation),
        )
        ...
        gap_scale *= float(gap_floor + (1.0 - gap_floor) * consensus)
        ...
        if selected_indices.size > full_keep and partial_keep > 1e-6:
            adjusted_partial_keep = partial_keep * float(fractional_floor + (1.0 - fractional_floor) * consensus)
