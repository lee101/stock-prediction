...
DEFAULT_PYTHON = Path(".venv312/bin/python")
DEFAULT_BACKENDS = ("codex",)
DEFAULT_FREQUENCIES = ("hourly",)
...
def _append_leaderboard_row(path: Path, record: TurnRecord) -> None:
    ...
    header = (
        ...
        "total_seconds\tpeak_vram_mb\tscenario_count\ttotal_trade_count\ttrain_samples\tnum_steps\t"
    ...
    with path.open("a", encoding="utf-8") as handle:
        ...
        fields = [
            ...
            "" if record.train_samples is None else str(record.train_samples),
...
def parse_train_log(log_path: Path) -> ParsedTrainMetrics:
    ...
    for line in text.splitlines():
        ...
        elif key == "train_samples" and isinstance(value, float):
            metrics.train_samples = int(value)
        ...
        elif key == "hold_bars" and isinstance(value, float):
            metrics.hold_bars = int(value)
    return metrics
...
def resolve_python_path(raw_path: str, *, repo_root: Path) -> Path:
    ...
    # Keep the virtualenv interpreter path intact instead of resolving the symlink
    # back to the base system python, which drops the venv package context.
...
def _history_summary_lines(history_rows: list[dict[str, Any]]) -> list[str]:
    if not history_rows:
        return ["No prior turns recorded yet in this scheduler history."]
    lines: list[str] = []
    for row in history_rows:
        ...
        lines.append(
            f"- turn {row.get('turn_index')} | backend={row.get('backend')} | freq={row.get('frequency')} "
...
def build_turn_prompt(
    *,
    ...
    train_log = run_dir / f"train_{frequency}.log"
    result_json = run_dir / "agent_result.json"
    experiment_code_root = repo_root / DEFAULT_EXPERIMENT_CODE_ROOT
    prompt = f"""
You are running one bounded optimization turn for the autoresearch-style stock planner in this repository.

Repository root: {repo_root}
Target frequency for this turn: {frequency}
Python interpreter: {python_path}
Run directory for this turn: {run_dir}
Deterministic experiment bundle directory for this turn: {experiment_dir}
Preferred root for any new importable experiment code: {experiment_code_root}
Required training log path: {train_log}
Structured result file path: {result_json}

Primary goal:
- improve `robust_score`

Secondary goal:
- track and ideally reduce `val_loss`

Hard rules:
- Keep the experiment deterministic and replayable.
- If you need more than a tiny one-file change, isolate new code under `src/autoresearch_stock/experiments/<experiment_slug>/` and keep `src/autoresearch_stock/train.py` as a small dispatcher.
- Mirror any hypothesis/config/replay notes into the experiment bundle directory shown above.
- Do not edit `src/autoresearch_stock/prepare.py`.
- Do not fake the benchmark. Run one real five-minute training run with the fixed harness.
- Use the exact interpreter shown above.
- Keep changes minimal and production-defensible.

Read first:
- `src/autoresearch_stock/README.md`
- `src/autoresearch_stock/program.md`
...
3. Run the real training command above exactly once after your edit.
4. Parse the summary lines from the log.
5. Return the structured JSON response only.

Structured response requirements:
- `status`: `"success"`, `"no_change"`, or `"failed"`
- `summary`: one short sentence
- `touched_files`: changed paths
- `train_log`: path to the log file you used
- `robust_score`, `val_loss`, `training_seconds`, `total_seconds`, `peak_vram_mb`, `num_steps`: parsed numbers or null
- `notes`: short flat list of key observations
""".strip()
    if prompt_sections:
        prompt += "\n\nLoaded prompt packs:\n"
    ...
    *,
    ...
    if backend == "codex":
        ...
        if result_path.exists() and not stdout_path.exists():
            # keep a copy of stdout for consistency
            ...
            if backend == "claude" and not allow_claude:
                backend_statuses.append(
                    BackendStatus(
                        name=backend,
                        available=False,
                        reason="Claude backend disabled by default on this machine; pass --allow-claude after fixing CLI auth.",
                    )
                )
...
def run_turn(
    *,
    ...
    started_at = _utc_now()
    turn_name = f"turn_{selection.turn_index:04d}_{selection.backend}_{selection.frequency}_{_timestamp_token(started_at)}"
    ...
    try:
        ...
        result_error = f"invalid agent JSON output at {agent_result_path}: {exc}"
    ...
    if isinstance(raw_train_log, str) and raw_train_log.strip():
        ...
        default_path = run_dir / f"train_{selection.frequency}.log"
    ...
    _copy_if_exists(train_log_path, experiment_dir / f"train_{selection.frequency}.log")
    ...
    record = TurnRecord(
        ...
        train_samples=parsed_metrics.train_samples,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Schedule repeated Codex/Claude optimization turns for autoresearch_stock.")
    ...
    parser.add_argument("--frequencies", default="hourly")
...
def main() -> int:
    ...
    if not available_backends:
        raise SystemExit("No usable agent backends are available. Check analysis/autoresearch_stock_agent/backend_status.json.")
    ...
    while True:
        ...
        print(
            f"turn={record.turn_index} backend={record.backend} frequency={record.frequency} "
