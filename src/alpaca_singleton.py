...
def _buy_memory_path(account_name: str) -> Path:
    safe = str(account_name).strip().lower().replace("/", "_").replace(" ", "_")
...
def enforce_live_singleton(
    *,
    ...
    if _env_flag("ALPACA_SINGLETON_OVERRIDE"):
        _write_marker("enforce_live_singleton",
...
def guard_sell_against_death_spiral(
    ...
    *,
    ...
    with _STATE_MU:
        state = _STATE
    account_name = state.account_name if state is not None else "alpaca_live_writer"
    tol_bps = (
        float(tolerance_bps)
    ...
    if _env_flag("ALPACA_DEATH_SPIRAL_OVERRIDE"):
        _write_marker(
