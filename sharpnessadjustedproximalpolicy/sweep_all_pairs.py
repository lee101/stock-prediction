...
Usage:
    python -m sharpnessadjustedproximalpolicy.sweep_all_pairs
    python -m sharpnessadjustedproximalpolicy.sweep_all_pairs --epochs 15 --top-k 3
"""
...
def main():
    ...
    output_json = Path(f"sharpnessadjustedproximalpolicy/allpairs_results_{timestamp}.json")
    output_csv = Path(f"sharpnessadjustedproximalpolicy/allpairs_leaderboard_{timestamp}.csv")
    ...
    for si, symbol in enumerate(symbols):
        for ci, config in enumerate(configs):
            ...
            with open(output_csv, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["symbol", "config", "best_epoch", "val_sortino", "val_return", "val_score", "sharpness_ema", "wd_scale", "error"])
                for r in sorted(all_results, key=lambda x: x.get("best_val_sortino", -999), reverse=True):
                    writer.writerow([
                        ...
                        f"{r.get('final_lr_scale', 1):.2f}" if not r.get("error") else "",
                        r.get("error", ""),
                    ])
