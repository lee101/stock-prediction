...
Usage:
    python -m sharpnessadjustedproximalpolicy.run_forever
    python -m sharpnessadjustedproximalpolicy.run_forever --symbols DOGEUSD BTCUSD --max-rounds 50
"""
...
LOG_DIR = Path("sharpnessadjustedproximalpolicy") / "analysis"
PROGRESS_DIR = Path("sharpnessadjustedproximalpolicy")
CHECKPOINT_DIR = Path("sharpnessadjustedproximalpolicy") / "checkpoints"
...
def write_progress(result: dict, round_num: int, is_best: bool):
    if is_best:
        idx = 1
        while (PROGRESS_DIR / f"sap_progress{idx}.md").exists():
            idx += 1
        path = PROGRESS_DIR / f"sap_progress{idx}.md"
...
- LR Scale: {result.get('final_lr_scale', 1):.3f}
...
"""
        ...
        path = PROGRESS_DIR / "sap_progress_failed.md"
        with open(path, "a") as f:
            f.write(
...
def main():
    ...
    leaderboard_path = LOG_DIR / "sap_leaderboard.json"
    log_path = LOG_DIR / "sap_forever_log.jsonl"
