...
Usage:
    python -m sharpnessadjustedproximalpolicy.evaluate --checkpoint path/to/epoch_005.pt --symbol DOGEUSD
    python -m sharpnessadjustedproximalpolicy.evaluate --checkpoint-dir path/to/run/ --symbol DOGEUSD
"""
...
def evaluate_checkpoint(
    ...
    results["lr_scale"] = sharpness_info.get("lr_scale", 1)
...
def main():
    ...
    if args.checkpoint:
        ...
        paths = sorted(Path("sharpnessadjustedproximalpolicy/checkpoints").rglob("epoch_*.pt"))
    ...
    out = Path("sharpnessadjustedproximalpolicy") / "eval_results.json"
