...
Usage:
    python -m sharpnessadjustedproximalpolicy.sweep --symbol DOGEUSD
    python -m sharpnessadjustedproximalpolicy.sweep --symbol DOGEUSD --experiments periodic_rho005,baseline_adamw
    python -m sharpnessadjustedproximalpolicy.sweep --symbol DOGEUSD --all-symbols
"""
...
def build_configs(exp: dict, symbol: str, base_overrides: dict | None = None) -> tuple[TrainingConfig, SAPConfig]:
    ...
    if base_overrides:
        tc_kwargs.update(base_overrides)
...
def main():
    ...
    # Print summary table
    print(f"\n{'Name':<30} {'Symbol':<10} {'Sort':>8} {'Ret':>10} {'Ep':>4} {'Sharp':>8} {'Scale':>6}")
    print("-" * 86)
