...
from neuralhourlystocksv5.config import SimulationConfigStocksV5
from neuralhourlystocksv5.data import StockFeatureNormalizer, HOURLY_FEATURES_STOCKS_V5
...
class HourlyStockMarketSimulatorV5:
    """Walk-forward market simulator for V5 hourly trading."""

    def __init__(self, config: SimulationConfigStocksV5) -> None:
        self.config = config
    ...
    def run(
        self,
        model: HourlyStockPolicyV5,
        bars: pd.DataFrame,
        features: np.ndarray,
        normalizer: StockFeatureNormalizer,
        ...
        for i in range(sequence_length, len(bars) - 24):
            ...
            early_exit = evaluate_drawdown_vs_profit_early_exit(
                ...
                label="HourlyStockMarketSimulatorV5",
            )
...
def run_10day_validation(
    ...
    features: np.ndarray,
    normalizer: StockFeatureNormalizer,
    config: Optional[SimulationConfigStocksV5] = None,
    sequence_length: int = 168,
) -> BacktestResult:
    """
    ...
    simulator = HourlyStockMarketSimulatorV5(config)
