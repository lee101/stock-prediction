#!/usr/bin/env python3
"""Train BTC LoRA with MAE consistency metrics and preaug support."""
...
from preaug import get_augmentation, BaseAugmentation

DEFAULT_DATA_PATH = Path("trainingdata/csv/BTC.csv")
DEFAULT_OUTPUT_ROOT = Path("chronos2_finetuned")
DEFAULT_RESULTS_DIR = Path("hyperparams/btc_lora_consistent")
...
class ConsistencyMetrics:
    ...
    def consistency_score(self) -> float:
        """Lower is better. Penalizes high variance and max errors."""
...
def compute_consistency_metrics(
    ...
    if start_idx >= end_idx - prediction_length:
        logger.warning("Not enough data for eval: start={} end={} ctx={}", start_idx, end_idx, context_length)
...
def train_and_evaluate(cfg: TrainConfig, data_path: Path, output_root: Path) -> Dict[str, Any]:
    """Train model and compute consistency metrics."""
    from chronos2_trainer import TrainerConfig, _load_pipeline, _fit_pipeline, _prepare_inputs, _save_pipeline
    ...
    logger.info("Data: train={} val={} test={}", len(train_df), len(val_df), len(test_df))
    ...
    run_name = f"BTC_lora_{cfg.preaug}_ctx{cfg.context_length}_lr{cfg.learning_rate:.0e}_r{cfg.lora_r}_{time.strftime('%Y%m%d_%H%M%S')}"
...
def main():
    ...
    parser.add_argument("--preaug", type=str, default="baseline", choices=["baseline", "percent_change", "log_returns", "differencing", "robust_scaling"])
    ...
    logger.info("Training BTC LoRA: ctx={} preaug={} lr={:.0e}", cfg.context_length, cfg.preaug, cfg.learning_rate)

    result = train_and_evaluate(cfg, args.data_path, args.output_root)
    ...
    logger.info("Val MAE%: mean={:.4f} std={:.4f} max={:.4f}", result["val"]["mae_percent_mean"], result["val"]["mae_percent_std"], result["val"]["mae_percent_max"])
    logger.info("Val consistency score: {:.4f}", result["val_consistency_score"])
    logger.info("Test MAE%: mean={:.4f} std={:.4f} max={:.4f}", result["test"]["mae_percent_mean"], result["test"]["mae_percent_std"], result["test"]["mae_percent_max"])
