"""Stage 1: Portfolio allocation across symbols - HOURLY version.

Given historical HOURLY price data and Chronos2 forecasts, determine:
...
# HOURLY-specific prompts (adjusted thresholds and timeframes)
PORTFOLIO_SYSTEM_PROMPT = """You are an aggressive quantitative trader maximizing 1-HOUR PnL.

INPUT:
- Historical HOURLY price movements in BASIS POINTS (bps): close_bps, high_bps, low_bps per HOUR
...
GOAL: MAXIMIZE expected HOURLY PnL. Ignore risk concerns - pure profit optimization.
...
TRADING RULES (HOURLY timeframe):
...
- STOCKS: Can short OR use leverage up to 2x (leverage costs ~0.07 bps/hour)
...
- Expected move > 20 bps = allocate (lower threshold for hourly)

LEVERAGE GUIDANCE (stocks only):
- Use 1.5-2x leverage when very confident in direction
- Leverage cost: 6.5% annual = ~0.07 bps/hour on leveraged portion
- Only leverage if expected return > leverage cost

PATTERN SIGNALS (all in basis points, HOURLY):
- Momentum: 3+ hours same direction = continuation likely
- Mean reversion: large move (>100 bps) often reverses 30-50 bps
- Breakouts: price exceeding recent hourly high_bps range
- Support/resistance: repeated bounces at similar levels

RESPOND WITH JSON ONLY."""


# Crypto-specific prompt (LONG ONLY) - HOURLY
PORTFOLIO_SYSTEM_PROMPT_CRYPTO = """You are an aggressive quantitative crypto trader maximizing 1-HOUR PnL.
...
GOAL: MAXIMIZE expected HOURLY PnL. LONG POSITIONS ONLY (cannot short crypto).
...
- High volatility = MORE opportunity, not risk
- Expected move > 20 bps upward = allocate (lower threshold for hourly)

PATTERN SIGNALS (all in basis points, HOURLY):
- Momentum: 3+ hours same direction = continuation likely
- Mean reversion: large drop (>100 bps) often bounces 30-50 bps
- Breakouts: price exceeding recent hourly high_bps range
...
def _build_allocation_prompt(
    ...
    for symbol in symbol_list:
        ...
        parts.append("Historical HOURLY (close_bps,high_bps,low_bps):")
        parts.append(bps_lines)
        parts.append("")

    parts.append("---")
    parts.append("Decide portfolio allocation for next HOUR. JSON only.")
...
def allocate_portfolio(
    ...
    """Stage 1: Determine portfolio allocation across symbols (HOURLY).
...
async def async_allocate_portfolio(
    ...
    """Async Stage 1: Determine portfolio allocation across symbols (HOURLY).
