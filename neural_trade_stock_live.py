...
class NeuralTradingLoop:
    ...
    def _tick(self) -> None:
        ...
        logger.info("Generated %d plans (aggregate weight %.3f)", len(plans), total_weight)
    ...
    def _dispatch_plan(self, neural_plan: NeuralPlan, account_equity: float) -> None:
        ...
        qty = max(qty, min_qty)
        if qty <= 0:
            logger.debug("Skipping %s plan due to zero qty (notional %.2f).", symbol, target_notional)
            return
        logger.info(
            "Dispatching %s plan | buy @ %.4f qty=%.4f weight=%.3f priority=%d",
            symbol,
            plan.buy_price,
            qty,
            plan.trade_amount,
            neural_plan.priority,
        )
...
def _resolve_checkpoint_path(args: argparse.Namespace) -> Path:
    ...
    if args.checkpoint_run_dir:
        ...
        logger.info(
            "Resolved best checkpoint %s (val_loss %.6f epoch %s)",
            best_path,
            best.get("val_loss"),
            best.get("epoch"),
        )
...
def main() -> None:
    ...
    def _handle_signal(signum, _frame):  # pragma: no cover - signal handling
        logger.warning("Received signal %s - stopping neural trading loop.", signum)
