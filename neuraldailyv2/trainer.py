"""V2 Trainer with temperature annealing and dual optimizer.

Key V2 features:
- Temperature annealing: Start with soft fills, anneal toward binary
- Dual optimizer: Muon for matrices, AdamW for embeddings/head
- Uses unified simulation module
"""
...
from neuraldailyv2.simulation import compute_loss, simulate_trades
...
class NeuralDailyTrainerV2:
    """V2 trainer with temperature annealing and unified simulation."""

    def __init__(
        self,
        config: DailyTrainingConfigV2,
        data_module: DailyDataModuleV2,
    ):
        self.config = config
        ...
        self.model = MultiSymbolPolicyV2(policy_config).to(self.device)
        ...
        logger.info(f"V2 Trainer initialized on {self.device}")
    ...
    def train(self) -> Dict[str, Any]:
        ...
        history = {"train_loss": [], "val_loss": [], "val_sortino": [], "temperature": []}
        ...
        *,
        ...
        with context:
            for batch in loader:
                ...
                total_sortino += metrics["sortino"]
                total_return += metrics["annual_return"]
                num_batches += 1

        return {
            "loss": total_loss / max(1, num_batches),
            "sortino": total_sortino / max(1, num_batches),
            "annual_return": total_return / max(1, num_batches),
        }
    ...
    def _save_checkpoint(self, epoch: int, metrics: Dict[str, float]) -> Path:
        ...
        logger.info(f"Checkpoint saved: {checkpoint_path} (sortino={metrics['sortino']:.4f})")
        ...
        for ckpt_path in sorted(self.checkpoint_dir.glob("epoch_*.pt")):
            try:
                ...
                manifest["checkpoints"].append({
                    "path": ckpt_path.name,
                    "val_loss": -ckpt["metrics"]["sortino"],  # Negative for sorting
                    "epoch": ckpt["epoch"],
