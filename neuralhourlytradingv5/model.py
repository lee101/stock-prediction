"""V5 Transformer model for hourly crypto trading.
...
class HourlyCryptoPolicyV5(nn.Module):
    """
    V5 Policy network for hourly crypto trading.
    ...
    where offset >= 8bps, making price crossing mathematically impossible.
    """
    ...
    def forward(self, features: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Output logits
        outputs = {
            "position_length_logits": self.position_length_head(
                h_pooled
            ),  # (batch, 25)
            "position_size_logits": self.position_size_head(h_pooled),  # (batch, 1)
            "buy_offset_logits": self.buy_offset_head(h_pooled),  # (batch, 1)
            "sell_offset_logits": self.sell_offset_head(h_pooled),  # (batch, 1)
        }
    ...
    def decode_actions(
        ...
        CRITICAL SAFETY:
        - buy_price = midpoint * (1 - offset) where offset >= 8bps
        - sell_price = midpoint * (1 + offset) where offset >= 8bps
        ...
        Args:
            ...
            reference_close: (batch,) - previous bar's close price (midpoint)
            temperature: Temperature for softmax (lower = harder decisions)

        Returns:
            Dict with decoded actions:
            - position_length: 0-24 hours (0 = skip this trade)
            - position_size: 0-1 fraction of capital
            - buy_price: Always < reference_close - 8bps
            - sell_price: Always > reference_close + 8bps
        """
        config = self.config
        ...
        # 25-class softmax: class 0 = skip, classes 1-24 = hold hours
        ...
        position_length = (length_probs * hours).sum(dim=-1)  # (batch,)

        # Position size: sigmoid [0, 1]
        position_size = torch.sigmoid(outputs["position_size_logits"]).squeeze(-1)

        # ========== CRITICAL: Fee-Aware Price Decoding ==========
        # Raw offsets from sigmoid [0, 1]
        buy_raw = torch.sigmoid(outputs["buy_offset_logits"]).squeeze(-1)
        sell_raw = torch.sigmoid(outputs["sell_offset_logits"]).squeeze(-1)

        # Scale to [min_offset, max_offset]
        # min_offset = 8bps (fee buffer - creates dead zone around midpoint)
        # max_offset = 3% (reasonable limit order distance)
        ...
        # CRITICAL: Prices are always offset from midpoint
        # buy_price = midpoint * (1 - offset) where offset >= 8bps
        # This means buy_price <= midpoint * 0.9992 (always below fee threshold)
        buy_price = reference_close * (1.0 - buy_offset)
        sell_price = reference_close * (1.0 + sell_offset)
        # =========================================================

        return {
            "position_length": position_length,  # 0-24 hours (0 = skip)
            "position_size": position_size,  # 0-1 fraction of capital
            "buy_price": buy_price,  # Always < reference_close - 8bps
            "sell_price": sell_price,  # Always > reference_close + 8bps
            "buy_offset": buy_offset,  # Offset from midpoint
            "sell_offset": sell_offset,  # Offset from midpoint
            "length_probs": length_probs,  # For loss computation
        }
    ...
    def get_hard_actions(
        ...
        Uses argmax for position_length instead of soft weighted sum.
        """
        config = self.config
        device = reference_close.device
        ...
        # Position size: sigmoid (same as soft)
        position_size = torch.sigmoid(outputs["position_size_logits"]).squeeze(-1)

        # Prices: same as soft (offset-based is already deterministic)
        buy_raw = torch.sigmoid(outputs["buy_offset_logits"]).squeeze(-1)
        sell_raw = torch.sigmoid(outputs["sell_offset_logits"]).squeeze(-1)
