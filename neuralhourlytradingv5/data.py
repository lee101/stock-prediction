"""V5 Data module for hourly crypto trading with lookahead support.

Key features:
- 168-hour sequences (1 week)
- 24-hour lookahead for simulation
...
class HourlyDatasetV5(Dataset):
    """Dataset with 168-hour sequences and 24-hour lookahead for simulation."""
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            # Lookahead for simulation (24 hours)
...
class HourlyDataModuleV5:
    """Data module for V5 hourly trading with multi-symbol support."""

    def __init__(self, config: DatasetConfigV5) -> None:
        self.config = config
        self.feature_columns = tuple(config.feature_columns or HOURLY_FEATURES_V5)
        ...
        self.train_dataset = HourlyDatasetV5(
            train_frame,
            norm_train,
        ...
        self.val_dataset = HourlyDatasetV5(
            val_frame,
            norm_val,
            config.sequence_length,
            config.lookahead_hours,
    ...
    def _prepare_frame(self) -> pd.DataFrame:
        ...
        # Fill remaining NaN with 0 for Chronos delta features (no forecast = no expected change)
        ...
        # Drop rows with NaN in non-Chronos features (requires some history)
    ...
    def _load_forecasts(self) -> pd.DataFrame:
        ...
        if not cache_path.exists():
            ...
            # Features will be filled with defaults
            return pd.DataFrame(
                columns=[
                    "timestamp",
                    "symbol",
...
class MultiSymbolDataModuleV5:
    """Data module for training on multiple crypto pairs."""

    def __init__(self, symbols: Sequence[str], config: DatasetConfigV5) -> None:
        cleaned: List[str] = []
        seen: set = set()
        ...
        self.modules: Dict[str, HourlyDataModuleV5] = {}

        # Create data module for each symbol
        for symbol in self.symbols:
            symbol_config = DatasetConfigV5(
                symbols=(symbol,),
            ...
            try:
                module = HourlyDataModuleV5(symbol_config)
