...
# 20-model ensemble stored in prod_ensemble/ (protected from *_screen/ deletion pattern)
# Members: tp10+s15+s36+gamma_995+muon_wd_005+h1024_a40+s1731+gamma995_s2006+s1401+s1726+s1523+s2617+s2033+s2495+s1835+s2827+s2722+s3668+s3411+s4009
...
# s4009 added 2026-03-29: +4.4% delta vs 19-model — 20-model: 0/111 neg, p10=48.5%
...
# 21-model bar: 21-model exhaustive p10 >= 48.5% @fill_bps=5 (encoder_norm-correct methodology)
...
# REJECTED against 17/18/19-model: 100+ seeds tested (see batch_new_*.log)
DEFAULT_EXTRA_CHECKPOINTS = [
    ...
    "pufferlib_market/prod_ensemble/s4009.pt",
]
...
class InMemoryTradingServerClient:
    ...
    def claim_writer(self, *, ttl_seconds: int = 120) -> dict[str, object]:
