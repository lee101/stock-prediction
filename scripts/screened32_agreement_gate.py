...
def _parse_int_grid(raw: str, *, name: str, min_value: int) -> list[int]:
    ...
    for part in str(raw).split(","):
        ...
        if parsed < int(min_value):
            raise ValueError(f"{name} contains value below {int(min_value)}: {parsed}")
        if parsed in seen:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        counts = _parse_int_grid(args.min_agree_counts, name="min_agree_counts", min_value=0)
    except ValueError as exc:
        print(f"screened32_agreement_gate: {exc}", file=sys.stderr)
        return 2

    val_path = Path(args.val_data)
    if not val_path.exists():
        print(f"screened32_agreement_gate: val not found: {val_path}", file=sys.stderr)
        return 2
    ...
    if not start_indices:
        print(
            f"screened32_agreement_gate: val too short for window_days={window_days} (T={total_steps})",
            file=sys.stderr,
        )
        return 2
