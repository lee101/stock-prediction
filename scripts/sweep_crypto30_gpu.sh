#!/bin/bash
# Crypto30 GPU training sweep. C binding rebuilt on daisy with numpy 2.x.
# GPU training now works correctly (reward_scale/clip applied via C binding).
...
CKPT_ROOT="pufferlib_market/checkpoints/crypto30_gpu"
...
echo "=== crypto30 GPU sweep ==="
echo

# h=256 baseline (proven config) -- 5 seeds
...
# h=256 + weight decay
for s in 1 2 3; do
  run_config "h256_wd005"     256  0.02  0.005  0.01  $s
done
...
# h=512
for s in 1 2 3; do
  run_config "h512"           512  0.02  0.0    0.01  $s
done

# h=1024
for s in 1 2 3; do
  run_config "h1024"          1024 0.02  0.0    0.01  $s
done

# h=256 higher entropy
for s in 1 2; do
  run_config "h256_ent03"     256  0.02  0.0    0.03  $s
done
