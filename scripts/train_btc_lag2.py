#!/usr/bin/env python3
"""Quick BTC training with lag=2, new features, binary fills.
Designed to run fast on GPU with limited VRAM.
"""
import sys
import time
...
def train_symbol(symbol: str, epochs: int = 20, seed: int = 42):
    from datetime import datetime
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_name = f"{symbol}_lag2_s{seed}_{ts}"
...
if __name__ == "__main__":
    ...
    result = train_symbol(args.symbol, args.epochs, args.seed)
