#!/usr/bin/env python3
"""Test symbol blacklisting for crypto30 ensemble."""
from __future__ import annotations
import collections
import sys
from pathlib import Path
import numpy as np
import torch

REPO = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(REPO))

from pufferlib_market.hourly_replay import read_mktd, MktdData, INITIAL_CASH, P_CLOSE, simulate_daily_policy
from pufferlib_market.evaluate_holdout import load_policy, _mask_all_shorts

VAL_BIN = REPO / "pufferlib_market/data/crypto30_daily_val.bin"
PROD = [
    REPO / "pufferlib_market/checkpoints/crypto30_ensemble/s2.pt",
    REPO / "pufferlib_market/checkpoints/crypto30_ensemble/s19.pt",
    REPO / "pufferlib_market/checkpoints/crypto30_ensemble/s21.pt",
    REPO / "pufferlib_market/checkpoints/crypto30_ensemble/s23.pt",
]


def eval_with_blacklist(data, policies, S, btc_close, blacklist=None, regime_ma=15):
    T = data.num_timesteps
    pending = collections.deque(maxlen=3)
    step_counter = [0]
    symbols = list(data.symbols)

    # Build blacklist mask: actions to block
    blocked_actions = set()
    if blacklist:
        for sym in blacklist:
            if sym in symbols:
                idx = symbols.index(sym)
                blocked_actions.add(idx + 1)  # action = sym_idx + 1

    def policy_fn(obs):
        t = step_counter[0]
        step_counter[0] += 1

        if regime_ma > 0 and t >= regime_ma:
            ma = float(np.mean(btc_close[t-regime_ma+1:t+1]))
            if btc_close[t] <= ma:
                action_now = 0
                pending.append(action_now)
                if len(pending) <= 2:
                    return 0
                return pending.popleft()

        obs_t = torch.from_numpy(obs.astype(np.float32)).view(1, -1)
        with torch.no_grad():
            probs = [torch.softmax(p(obs_t)[0], dim=-1) for p in policies]
            avg = torch.stack(probs).mean(dim=0)
            log_avg = torch.log(avg + 1e-8)
            # Mask shorts
            if log_avg.shape[-1] > 1 + S:
                log_avg[0, 1+S:] = float("-inf")
            # Mask blacklisted symbols
            for a in blocked_actions:
                log_avg[0, a] = float("-inf")
            action_now = int(torch.argmax(log_avg, dim=-1).item())

        pending.append(action_now)
        if len(pending) <= 2:
            return 0
        return pending.popleft()

    result = simulate_daily_policy(
        data, policy_fn, max_steps=T - 1,
        fee_rate=0.001, slippage_bps=5.0,
        fill_buffer_bps=5.0, max_leverage=1.0,
        periods_per_year=365.0, enable_drawdown_profit_early_exit=False,
    )
    eq = np.array(result.equity_curve) if result.equity_curve is not None and len(result.equity_curve) > 0 else np.ones(T) * INITIAL_CASH
    ret = eq[-1] / eq[0] - 1
    peak = np.maximum.accumulate(eq)
    dd = float(np.max((peak - eq) / np.maximum(peak, 1e-8)))
    dr = np.diff(eq) / np.maximum(eq[:-1], 1e-8)
    neg = dr[dr < 0]
    sort = float(np.mean(dr) / np.sqrt(np.mean(neg**2)) * np.sqrt(365)) if len(neg) > 0 else 0
    return ret, sort, dd


def eval_walkforward(data_full, policies, S, btc_close, blacklist=None, regime_ma=15):
    T = data_full.num_timesteps
    window, stride = 30, 5
    rets = []
    for w_start in range(0, T - window + 1, stride):
        w_end = w_start + window
        data_w = MktdData(
            version=data_full.version,
            symbols=list(data_full.symbols),
            features=data_full.features[w_start:w_end].copy(),
            prices=data_full.prices[w_start:w_end].copy(),
            tradable=data_full.tradable[w_start:w_end].copy() if data_full.tradable is not None else None,
        )
        btc_slice = btc_close  # Use global for regime
        ret, _, _ = eval_with_blacklist(data_w, policies, S, btc_slice, blacklist, regime_ma)
        rets.append(ret)
    return np.array(rets)


def main():
    data = read_mktd(str(VAL_BIN))
    S = data.num_symbols
    T = data.num_timesteps
    print(f"data: {T}d x {S}sym\n")

    btc_idx = next(i for i, s in enumerate(data.symbols) if "BTC" in s)
    btc_close = data.prices[:, btc_idx, P_CLOSE].astype(np.float64)

    policies = []
    for cp in PROD:
        loaded = load_policy(cp, S, features_per_sym=16, device=torch.device("cpu"))
        policies.append(loaded.policy)

    print("=== Full-period test (192d, 5bp slip, MA15 regime) ===")
    blacklists = [
        ("None", None),
        ("TRX", ["TRXUSDT"]),
        ("TRX+BCH", ["TRXUSDT", "BCHUSDT"]),
        ("TRX+BCH+INJ", ["TRXUSDT", "BCHUSDT", "INJUSDT"]),
        ("all_neg (TRX+BCH+INJ+MATIC)", ["TRXUSDT", "BCHUSDT", "INJUSDT", "MATICUSDT"]),
    ]

    print(f"{'Blacklist':<35} {'Ret%':>10} {'Sort':>8} {'DD%':>8}")
    print("-" * 65)
    for label, bl in blacklists:
        r, s, d = eval_with_blacklist(data, policies, S, btc_close, bl, regime_ma=15)
        print(f"{label:<35} {r*100:>+10.2f}% {s:>8.2f} {d*100:>8.2f}%")

    # Walk-forward for top configs
    print(f"\n=== Walk-forward (33 x 30d, 5d stride) ===")
    print(f"{'Blacklist':<35} {'Mean%':>10} {'Med%':>10} {'%Pos':>8} {'Min%':>10}")
    print("-" * 75)
    for label, bl in blacklists[:3]:
        rets = eval_walkforward(data, policies, S, btc_close, bl, regime_ma=15)
        print(f"{label:<35} {rets.mean()*100:>+10.2f}% {np.median(rets)*100:>+10.2f}% "
              f"{100*(rets>0).mean():>7.0f}% {rets.min()*100:>+10.2f}%")


if __name__ == "__main__":
    main()
