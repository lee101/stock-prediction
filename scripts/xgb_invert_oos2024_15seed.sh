#!/usr/bin/env bash
# 15-seed bonferroni re-test of regime-inversion (bottom-N long) on oos2024
# fold. 5-seed claim: BOTTOM-N long returns +6.13%/mo @ lev=1, +8.58-9.38%/mo
# @ lev=2 on OOS 2025-07→2026-04 (tariff crash = cross-sectional regime flip).
# If this survives 15 seeds on BOTH folds, it's the first true-OOS edge of
# the post-stale-fix era.
set -euo pipefail
cd "$(dirname "$0")/.."
# shellcheck disable=SC1091
source .venv/bin/activate

OUT_DIR="analysis/xgbnew_daily/sweep_20260422_invert_oos2024_15seed"
mkdir -p "$OUT_DIR"
