...
def _write_symbol_csv(
    ...
    *,
    ...
    frame = pd.DataFrame(
        {
            time_column: pd.date_range("2025-01-01", periods=24, freq="D", tz=timezone),
...
def test_evaluate_hf_checkpoint_filters_utc_timestamp_csv(tmp_path: Path) -> None:
    data_root = tmp_path / "trainingdata"
    data_root.mkdir()
    csv_path = _write_symbol_csv(data_root, time_column="timestamp", timezone="UTC")
    ...
    checkpoint_dir = tmp_path / "hf_run_utc"
    checkpoint_dir.mkdir()
    processor_path = checkpoint_dir / "data_processor.pkl"
    processor.save_scalers(str(processor_path))
    ...
    baseline = evaluate_hf_checkpoint(
        checkpoint_path=checkpoint_path,
        processor_path=processor_path,
        symbol="AAPL",
        data_root=data_root,
    ...
    filtered = evaluate_hf_checkpoint(
        checkpoint_path=checkpoint_path,
        processor_path=processor_path,
        symbol="AAPL",
        data_root=data_root,
    ...
    assert filtered.steps < baseline.steps
    assert filtered.total_return == 0.0
...
def test_evaluate_puffer_checkpoint_infers_policy_shape_from_checkpoint(tmp_path: Path) -> None:
    data_root = tmp_path / "trainingdata"
    data_root.mkdir()
    csv_path = _write_symbol_csv(data_root)
    ...
    checkpoint_path = tmp_path / "puffer_policy_mismatch.pt"
    torch.save(policy.state_dict(), checkpoint_path)

    summary_path = tmp_path / "puffer_summary_mismatch.json"
    summary_path.write_text(
        json.dumps(
            {
                "model_path": str(checkpoint_path),
...
def test_evaluate_puffer_checkpoint_filters_utc_timestamp_csv(tmp_path: Path) -> None:
    data_root = tmp_path / "trainingdata"
    data_root.mkdir()
    csv_path = _write_symbol_csv(data_root, time_column="timestamp", timezone="UTC")
    ...
    checkpoint_path = tmp_path / "puffer_policy_utc.pt"
    torch.save(policy.state_dict(), checkpoint_path)

    summary_path = tmp_path / "puffer_summary_utc.json"
    summary_path.write_text(
        json.dumps(
            {
                "model_path": str(checkpoint_path),
    ...
    baseline = evaluate_puffer_checkpoint(
        summary_json=summary_path,
        symbol="AAPL",
        data_root=data_root,
        device="cpu",
    )
    filtered = evaluate_puffer_checkpoint(
        summary_json=summary_path,
        symbol="AAPL",
        data_root=data_root,
        start_date="2025-01-08",
