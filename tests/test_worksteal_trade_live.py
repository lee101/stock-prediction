...
def test_trade_live_preview_run_exits_before_client_and_model_init(monkeypatch, capsys):
    ...
    assert "Command:" in out
    assert "command: python -m binance_worksteal.trade_live --symbols btcusd ethusd --max-symbols 1 --live --daemon --gemini --neural-model model.pt --neural-symbols solusd adausd --preview-run" in out
...
def test_trade_live_preview_run_summary_dash_redirects_human_output(monkeypatch, capsys):
    ...
    assert "command: python -m binance_worksteal.trade_live --symbols btcusd ethusd --max-symbols 1 --live --daemon --gemini --neural-model model.pt --neural-symbols solusd adausd --preview-run --summary-json -" in captured.err
...
def test_run_health_report_passes_symbol_metrics_cache_to_strategy_calls(monkeypatch, tmp_path):
    monkeypatch.setattr("binance_worksteal.trade_live.STATE_FILE", tmp_path / "live_state.json")
    monkeypatch.setattr("binance_worksteal.trade_live.LOG_FILE", tmp_path / "trade_log.jsonl")
    monkeypatch.setattr("binance_worksteal.trade_live.EVENTS_FILE", tmp_path / "events.jsonl")
    monkeypatch.setattr("binance_worksteal.trade_live.load_state", lambda: {})
    ...
    sentinel_metrics = {"BTCUSD": FakeMetrics()}
    calls = {"resolve": 0}
