...
def test_fused_mlp_correctness(device, cc, hidden_size, batch_size):
    ...
    if HAS_TRITON:
        ...
        # BF16 accumulates error proportional to the magnitude of the result
        # (not absolute).  For hidden=1024, values can reach ~700; absolute
        # tolerance of 0.1 would be too tight.  Check relative error < 2% instead.
        out_f = out.float()
        ref_f = ref.float()
        max_ref = ref_f.abs().max().item()
        max_err = (out_f - ref_f).abs().max().item()
        rel_err = max_err / (max_ref + 1e-6)
        assert rel_err < 0.02, (
            f"Fused MLP mismatch at CC {cc[0]}.{cc[1]}, hidden={hidden_size}, batch={batch_size}: "
            f"max_err={max_err:.4f}, max_ref={max_ref:.4f}, rel_err={rel_err:.4f}"
        )
    else:
        # Fallback path: verify PyTorch fallback is correct (FP32, tight tolerance)
        out = fused_mlp_relu(x, W1, b1, W2, b2)
