...
def test_run_once_uses_single_sweep_call_for_multi_thresholds(monkeypatch, tmp_path: Path) -> None:
    args = _build_args(tmp_path, skip_existing=False)
    args.recency_halflife_days = "0.0,1.0"
    args.sit_out_thresholds = "0.25,0.5"
    ...
    def fake_run(cmd: list[str], check: bool) -> None:
        ...
        calls.append(cmd)
        output_path = Path(cmd[cmd.index("--output") + 1])
        recency = float(cmd[cmd.index("--recency-halflife-days") + 1])
        payload = {
            ...
            "summaries": [
                {
                    ...
                    "recency_halflife_days": recency,
                    "sit_out_threshold": 0.25,
                    "min_sortino": 0.3 + recency,
                    "mean_sortino": 0.4 + recency,
                    "min_return_pct": 0.2,
                    "mean_return_pct": 0.3,
                    "mean_max_drawdown_pct": 0.2,
                    "min_num_buys": 2,
                ...
                },
                {
                    ...
                    "min_score_gap": 0.0,
                    "recency_halflife_days": recency,
                    "sit_out_threshold": 0.5,
                    "min_sortino": 0.6 + recency,
                    "mean_sortino": 0.7 + recency,
                    "min_return_pct": 0.25,
                    "mean_return_pct": 0.35,
                    "mean_max_drawdown_pct": 0.15,
                    "min_num_buys": 2,
                    "mean_num_buys": 2.0,
                },
            ],
    ...
    assert len(calls) == 2  # recency values only; thresholds handled inside each sweep call
    assert all(
        any(arg == "--sit-out-thresholds=0.25,0.5" for arg in cmd)
        for cmd in calls
    )
    assert rec["best"]["sit_out_threshold"] == 0.5
    assert rec["best"]["recency_halflife_days"] == 1.0


def test_run_once_passes_negative_thresholds_as_single_arg(monkeypatch, tmp_path: Path) -> None:
    args = _build_args(tmp_path, skip_existing=False)
    args.sit_out_thresholds = "-0.0015,-0.001"
    ...
    assert calls
    assert any(arg == "--sit-out-thresholds=-0.0015,-0.001" for arg in calls[0])
