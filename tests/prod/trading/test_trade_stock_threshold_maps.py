...
def reset_threshold_caches(monkeypatch):
    ...
    monkeypatch.setattr(env_utils, "_DRAW_CAPS_CACHE", None, raising=False)
    monkeypatch.setattr(env_utils, "_DRAW_RESUME_CACHE", None, raising=False)
...
def test_parse_threshold_map_supports_symbol_and_strategy_specific_entries(monkeypatch):
    ...
    parsed = env_utils._parse_threshold_map("TEST_THRESHOLD_ENV")
...
def test_lookup_threshold_applies_precedence(monkeypatch):
    ...
    primary = env_utils._lookup_threshold("TEST_THRESHOLD_ENV", "SPY", "maxdiff")
    symbol_only = env_utils._lookup_threshold("TEST_THRESHOLD_ENV", "SPY", "probe")
    strategy_only = env_utils._lookup_threshold("TEST_THRESHOLD_ENV", "QQQ", "maxdiff")
    default_value = env_utils._lookup_threshold("TEST_THRESHOLD_ENV", "QQQ", "probe")
...
def test_drawdown_cap_map_and_fallback(monkeypatch):
    ...
    cap_primary = env_utils._drawdown_cap_for("maxdiff", "SPY")
    cap_symbol = env_utils._drawdown_cap_for("probe", "SPY")
    cap_strategy = env_utils._drawdown_cap_for("maxdiff", "QQQ")
    cap_default = env_utils._drawdown_cap_for("probe", "QQQ")
...
def test_drawdown_resume_map_and_factor(monkeypatch):
    ...
    resume_primary = env_utils._drawdown_resume_for("maxdiff", cap=0.3, symbol="SPY")
    resume_symbol = env_utils._drawdown_resume_for("probe", cap=0.3, symbol="SPY")
    resume_strategy = env_utils._drawdown_resume_for("maxdiff", cap=0.3, symbol="QQQ")
    resume_factor = env_utils._drawdown_resume_for("probe", cap=0.5, symbol="QQQ")
