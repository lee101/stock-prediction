...
def test_simulate_deepseek_replanning_reuses_updated_snapshot(monkeypatch):
    ...
    assert result.total_return_pct == pytest.approx((result.ending_equity - result.starting_equity) / result.starting_equity, rel=1e-6)
    assert result.annualized_return_pct == pytest.approx(
        (result.ending_equity / result.starting_equity) ** (365 / len(result.steps)) - 1,
        rel=1e-6,
    )
    summary_text = result.summary()
    assert "Annualized return" in summary_text
...
def test_build_deepseek_messages_mentions_leverage_guidance():
    ...
    assert "4× intraday" in combined
    assert "≤2× gross leverage" in combined
