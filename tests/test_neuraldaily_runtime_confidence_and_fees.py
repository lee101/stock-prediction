...
def test_simulator_account_fraction_uses_notional_sizing() -> None:
    dates = pd.date_range("2024-01-01", periods=3, freq="D", tz="UTC")
    frame = pd.DataFrame(
        {
            "date": dates,
    ...
    runtime = DummyRuntime(_DummyBuilder(frame))
    simulator = NeuralDailyMarketSimulator(runtime, ("BTCUSD",), initial_cash=100.0, account_fraction=0.5)
    _, summary = simulator.run(days=1)

    expected_qty = 100.0 * 0.5 / 10.0
    expected_pnl = expected_qty * ((12.0 * (1 - 0.0008)) - (10.0 * (1 + 0.0008)))
    assert math.isclose(summary["pnl"], expected_pnl, rel_tol=0, abs_tol=1e-6)
