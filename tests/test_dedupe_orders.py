...
class FakeBinanceAPIError(Exception):
...
def test_error_code_from_attribute():
    exc = FakeBinanceAPIError(-2011, "Unknown order sent.")
...
def test_error_code_none_attribute():
    exc = FakeBinanceAPIError(None, "No message available")
    assert _binance_error_code(exc) is None
...
@patch.object(
    trade_binance_live_module, "_cancel_open_order", side_effect=FakeBinanceAPIError(-2011, "Unknown order sent.")
)
...
@patch.object(
    trade_binance_live_module, "_cancel_open_order", side_effect=FakeBinanceAPIError(-2013, "Order does not exist.")
)
...
@patch.object(
    trade_binance_live_module, "_cancel_open_order", side_effect=FakeBinanceAPIError(None, "No message available")
)
...
def test_multiple_orders_second_fails_2011(mock_cancel):
    mock_cancel.side_effect = [None, FakeBinanceAPIError(-2011, "Unknown order")]
