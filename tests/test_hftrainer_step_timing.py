...
def test_trainer_falls_back_to_cpu_when_auto_cuda_is_unavailable(
    tmp_path: Path,
    monkeypatch,
) -> None:
    seq_len = 8
    ...
    config.persistent_workers = False
    config.prefetch_factor = 2
    config.enable_benchmark_metrics = False
    config.output_dir = str(tmp_path / "out")
    config.logging_dir = str(tmp_path / "logs")
    config.cache_dir = str(tmp_path / "cache")
    ...
    monkeypatch.setattr("hftraining.train_hf.torch.cuda.is_available", lambda: True)
    monkeypatch.setattr(model, "to", flaky_to)

    trainer = HFTrainer(model=model, config=config, train_dataset=dataset, eval_dataset=None)
