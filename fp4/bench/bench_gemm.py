...
def _nvfp4_gemm(m: int, n: int, k: int) -> float | None:
    try:
        from fp4 import linear as fp4_linear  # type: ignore
    except Exception:
        return None
    fn = getattr(fp4_linear, "nvfp4_matmul", None) or getattr(fp4_linear, "matmul", None)
    if fn is None:
        return None
    try:
        a = torch.randn(m, k, device="cuda", dtype=torch.bfloat16)
        b = torch.randn(k, n, device="cuda", dtype=torch.bfloat16)
        sec = _time_gemm(lambda: fn(a, b))
        return _tflops(m, n, k, sec)
...
def main() -> int:
    ...
    print()
    header = f"{'shape':<22}{'BF16 TFLOPs':>14}{'FP8 TFLOPs':>14}{'NVFP4 TFLOPs':>16}"
    print(header)
    print("-" * len(header))
    for (m, n, k) in SHAPES:
        bf = _bf16_gemm(m, n, k)
        ...
        print(f"{str((m,n,k)):<22}{bf:>14.1f}{f8_s}{f4_s}")
    return 0
