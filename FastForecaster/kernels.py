...
def load_fastforecaster_extension(*, build_extension: bool, verbose: bool = False) -> Any | None:
    """Compile and load optional C++/CUDA kernels used by FastForecaster.
    ...
    with _LOCK:
        ...
        try:
            ...
            setattr(_EXTENSION, "_fastforecaster_has_cuda", has_cuda)
        except Exception as exc:
            print(f"[fastforecaster] Optional extension build failed, falling back to PyTorch kernels: {exc}")
            _EXTENSION = None
...
def _mae_forward(
    ...
    *,
    ...
    if use_cpp:
        extension = load_fastforecaster_extension(build_extension=build_extension)
        if extension is not None and hasattr(extension, "fast_mae"):
            has_cuda = bool(getattr(extension, "_fastforecaster_has_cuda", False))
