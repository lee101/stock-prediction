...
def _list_symbol_paths(config: FastForecasterConfig) -> list[tuple[str, Path]]:
    ...
    if skipped_invalid:
        ...
        print(
            f"[fastforecaster:data] Skipped {len(skipped_invalid)} invalid symbol filenames: {preview}{extra}"
        )
...
def build_data_bundle(config: FastForecasterConfig) -> DataBundle:
    ...
    if not series_records:
        raise RuntimeError("No symbols satisfied FastForecaster dataset requirements.")
