...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run FastForecaster seed sweeps for reproducible MAE ranking.")

    parser.add_argument("--dataset", choices=["hourly", "daily", "custom"], default="hourly")
    parser.add_argument("--data-dir", type=Path, default=None)
    parser.add_argument("--output-dir", type=Path, default=Path("FastForecaster") / "seed_sweeps")
...
def _logger_context(
    *,
    ...
    return WandBoardLogger(
        ...
        tensorboard_subdir=f"fastforecaster/seed_sweeps/{run_name}",
        enable_wandb=True,
        log_metrics=True,
...
def _seed_config(base_cfg: FastForecasterConfig, *, sweep_dir: Path, seed: int) -> FastForecasterConfig:
...
def main() -> None:
    ...
    run_name_prefix = base_cfg.wandb_run_name or f"fastforecaster_seed_sweep_{run_stamp}"

    print(f"[fastforecaster-seed-sweep] Running {len(seeds)} seeds: {', '.join(str(s) for s in seeds)}")
    results: list[dict[str, object]] = []

    for index, seed in enumerate(seeds, start=1):
        cfg = _seed_config(base_cfg, sweep_dir=sweep_dir, seed=seed)
        run_name = _seed_run_name(run_name_prefix, seed)
        print(f"[fastforecaster-seed-sweep] Seed {index}/{len(seeds)}: {seed}")

        try:
            with _logger_context(cfg=cfg, run_name=run_name) as metrics_logger:
                trainer = FastForecasterTrainer(cfg, metrics_logger=metrics_logger)
            ...
            results.append(row)
            print(
                "[fastforecaster-seed-sweep] Completed "
            ...
            results.append(row)
            print(f"[fastforecaster-seed-sweep] Seed {seed} failed: {exc}")
    ...
    if ok_results:
        ...
        print(
            "[fastforecaster-seed-sweep] Best seed: "
            f"{best['seed']} val_mae={best['summary']['best_val_mae']:.6f} "
            f"test_mae={best['summary']['test_mae']:.6f}"
        )

    print(f"[fastforecaster-seed-sweep] Results written to {out_path}")
