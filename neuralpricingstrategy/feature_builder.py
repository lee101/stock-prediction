...
class FeatureSpec:
    ...
    def from_dict(cls, data: Mapping[str, object]) -> "FeatureSpec":
        ...
        categorical_levels = {key: list(levels) for key, levels in data["categorical_levels"].items()}
...
class FeatureBuilder:
    ...
    def spec(self) -> FeatureSpec:
        if self._spec is None:
            raise RuntimeError("FeatureBuilder must be fitted before use.")
        return self._spec
    ...
    def fit(self, df: pd.DataFrame) -> FeatureSpec:
        ...
        for column in self.categorical_columns:
            ...
            categorical_levels[column] = sorted(series.unique().tolist())

        feature_order: List[str] = []
        if self.add_bias:
            feature_order.append("bias")
        feature_order.extend(self.numeric_columns)
        for column in self.categorical_columns:
    ...
    def transform(self, df: pd.DataFrame) -> np.ndarray:
        ...
        for column in spec.categorical_levels.keys():
            if column not in df.columns:
                df[column] = "UNKNOWN"
