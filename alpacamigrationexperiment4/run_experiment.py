...
def _resolve_maker_fee(
    ...
    if is_crypto_symbol(symbol):
        return float(CRYPTO_TRADING_FEE)
...
def train_model(
    ...
    *,
    ...
    maker_fee = _resolve_maker_fee(args.maker_fee, data.asset_meta.symbol, data.asset_meta.maker_fee)
...
def evaluate_model(
    *,
    ...
    fee_value = _resolve_maker_fee(maker_fee, data.asset_meta.symbol, data.asset_meta.maker_fee)
    sim = AlpacaMarketSimulator(
        SimulationConfig(
            ...
            allow_intrabar_round_trips=allow_intrabar_round_trips,
            max_round_trips_per_bar=max_round_trips_per_bar,
        )
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--allow-intrabar-round-trips",
        action="store_true",
        help="Allow multiple buy/sell cycles within a single bar when both levels are hit.",
    )
    parser.add_argument(
        "--max-round-trips-per-bar",
        type=int,
        default=1,
        help="Maximum number of round trips allowed per bar when intrabar cycling is enabled.",
    )
    ...
    result = evaluate_model(
        ...
        allow_intrabar_round_trips=args.allow_intrabar_round_trips,
        max_round_trips_per_bar=args.max_round_trips_per_bar,
