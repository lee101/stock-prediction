...
The default grid now runs 12 variants per iteration.
Each iteration now reuses a shared feature cache file to avoid rebuilding the same ETH cube per variant.
Leaderboard ranking uses a robust score that prioritizes `7d/30d` outperformance versus buy-and-hold under `5/10 bps` buffers and penalizes near-zero-activity policies.
