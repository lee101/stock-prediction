"""Wide-panel CVaR sweep using XGB ensemble top-K as universe_fn.

Mirrors `sweep_wide_momentum.py` but selects per-rebalance universe from a
pre-computed XGB panel-score cache (see `scripts/build_xgb_panel_scores_wide.py`).

Cell tag parallels the momentum sweep so results are directly comparable:
    kNNN_xgb_wmaxXX_LtarYY_raZZ_confAAA_cminBB_hdHH_feeFF_slipSS_asl_psl[_atsTT_ptsPP]_msMM
"""
...
from cvar_portfolio.sweep_wide_momentum import load_active_panel
...
def _run_cell(
    ...
    *,
    ...
    ms_tag = f"_ms{int(round(min_score*100)):03d}" if min_score > 0 else ""
    cell_tag = (
        f"k{int(top_k):03d}_xgb_"
        f"wmax{w_max:.2f}_Ltar{L_tar:.2f}_ra{risk_aversion:.2f}_"
        f"conf{confidence:.3f}_cmin{c_min:+.2f}_hd{int(hold_days):02d}_"
        f"fee{int(fee_bps):02d}_slip{int(slip_bps):02d}{stop_tag}{ts_tag}{ms_tag}"
    )
    ...
    universe_fn = make_universe_fn(panel_scores, top_k=top_k, min_score=min_score)
    res = run_backtest(
        prices,
        fit_window=252,
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=Path("analysis/cvar_portfolio/sweep_wide_xgb_topk"))
    ap.add_argument("--xgb-score-cache", type=Path, required=True,
                    help="Parquet with columns date, symbol, ensemble_score.")
    ...
    ap.add_argument("--topk-grid", nargs="+", type=int, default=[25])
    ...
    print(f"Loading XGB score cache: {args.xgb_score_cache}")
    ...
    cells = list(itertools.product(
        args.topk_grid, args.min_score_grid,
    ...
    for i, (k, ms, wmax, ltar, ra, conf, hd, asl, psl, ats, pts) in enumerate(cells, 1):
        cmin = (1.0 - float(ltar)) if args.auto_lever else 0.0
        print(f"[{i}/{len(cells)}] k={k} ms={ms:.2f} wmax={wmax:.2f} Ltar={ltar:.2f} "
              f"ra={ra:.2f} conf={conf:.3f} cmin={cmin:+.2f} hd={hd} "
              f"asl={asl:.0f}%% psl={psl:.0f}%% ats={ats:.0f}%% pts={pts:.0f}%%",
              flush=True)
        row = _run_cell(
            prices, panel_scores,
            w_max=wmax, L_tar=ltar, risk_aversion=ra, confidence=conf,
            ...
            top_k=k, min_score=ms,
        )
        rows.append({
            "cell": row["cell"], "top_k": k, "min_score": ms,
