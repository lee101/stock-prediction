...
public:
    ...
    // Override the per-step execution price used by PnL/cost-basis accounting.
    // Used by DPS action mode to mark trades to the limit fill price instead of
    // the bar close. Pass an undefined tensor (default-constructed) to clear and
    // restore the legacy SCALAR path (uses close). The tensor must be shape
    // [batch_size], float32, on the same device as the portfolio.
    void set_execution_price_override(const torch::Tensor& px) { execution_price_override_ = px; }
    void clear_execution_price_override() { execution_price_override_ = torch::Tensor(); }
    const torch::Tensor& execution_price_override() const { return execution_price_override_; }
...
private:
    ...
    torch::Tensor execution_price_override_;  // undefined => use close (SCALAR legacy)
