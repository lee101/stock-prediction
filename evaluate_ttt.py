...
def _get_ttt_params(policy: nn.Module) -> list[nn.Parameter]:
    """Return the subset of policy parameters adapted during TTT.

    Keep early feature extractors frozen and only adapt the final shared block
    plus the actor head. This matches the old lightweight TTT behavior while
    keeping updates stable on small calibration windows.
    """

    params: list[nn.Parameter] = []

    if isinstance(policy, TradingPolicy):
        if len(policy.encoder) >= 5 and hasattr(policy.encoder[4], "parameters"):
            params.extend(policy.encoder[4].parameters())
        if getattr(policy, "_use_encoder_norm", False) and hasattr(policy, "encoder_norm"):
            params.extend(policy.encoder_norm.parameters())
        params.extend(policy.actor.parameters())
        return params

    if isinstance(policy, ResidualTradingPolicy):
        if len(policy.blocks) > 0:
            params.extend(policy.blocks[-1].parameters())
        params.extend(policy.out_norm.parameters())
        params.extend(policy.actor.parameters())
        return params

    params.extend(policy.parameters())
    return params
...
def _do_ttt_update(
    *,
    ...
    del rewards_list, old_values_list
    if not obs_list or not actions_list or not ttt_params:
        return

    max_steps = max(1, int(ttt_update_steps))
    obs = torch.cat([tensor.to(device) for tensor in obs_list[-max_steps:]], dim=0)
    actions = torch.cat([tensor.to(device) for tensor in actions_list[-max_steps:]], dim=0).long()
    old_log_probs = None
    if old_log_probs_list:
        old_log_probs = torch.cat([tensor.to(device) for tensor in old_log_probs_list[-max_steps:]], dim=0).detach()

    optimizer = torch.optim.Adam(ttt_params, lr=float(ttt_lr))
    policy.train()
    for _ in range(max(1, int(ttt_epochs))):
        optimizer.zero_grad(set_to_none=True)
        _, log_prob, entropy, _value = policy.get_action_and_value(obs, actions)
        loss = -log_prob.mean() - 0.01 * entropy.mean()
        if old_log_probs is not None and old_log_probs.shape == log_prob.shape:
            loss = loss + float(kl_coef) * torch.mean((log_prob - old_log_probs) ** 2)
        loss.backward()
...
def _infer_policy_from_state_dict(state_dict: dict[str, torch.Tensor]) -> tuple[nn.Module, int, int]:
    if any(key.startswith("input_proj.") or key.startswith("blocks.") for key in state_dict):
        obs_size = int(state_dict["input_proj.weight"].shape[1])
        hidden = int(state_dict["input_proj.weight"].shape[0])
        num_actions = int(state_dict["actor.2.weight"].shape[0])
        block_ids = {
            int(key.split(".")[1])
        ...
        return ResidualTradingPolicy(obs_size, num_actions, hidden=hidden, num_blocks=num_blocks), obs_size, num_actions

    obs_size = int(state_dict["encoder.0.weight"].shape[1])
    hidden = int(state_dict["encoder.0.weight"].shape[0])
    num_actions = int(state_dict["actor.2.weight"].shape[0])
    return TradingPolicy(obs_size, num_actions, hidden=hidden), obs_size, num_actions


def _load_policy(checkpoint_path: str, obs_size: int, num_actions: int, device: torch.device):
    del obs_size, num_actions
    payload = torch.load(checkpoint_path, map_location=device, weights_only=False)
    state_dict = payload.get("model", payload)
    policy, inferred_obs_size, inferred_num_actions = _infer_policy_from_state_dict(state_dict)
    policy.load_state_dict(state_dict, strict=False)
    policy.obs_size = inferred_obs_size
    policy.num_actions = inferred_num_actions
    policy.to(device)
    policy.eval()
    return policy, payload


def _print_comparison_table(ttt_windows: list[dict], base_windows: list[dict]) -> None:
    if not ttt_windows and not base_windows:
        return

    ttt_returns = [float(window.get("total_return", 0.0)) for window in ttt_windows]
    base_returns = [float(window.get("total_return", 0.0)) for window in base_windows]
    ttt_mean = float(np.mean(ttt_returns)) if ttt_returns else float("-inf")
    base_mean = float(np.mean(base_returns)) if base_returns else float("-inf")
    winner = "TTT" if ttt_mean >= base_mean else "Baseline"
    print(f"TTT mean return: {ttt_mean:+.4f}")
    print(f"Baseline mean return: {base_mean:+.4f}")
