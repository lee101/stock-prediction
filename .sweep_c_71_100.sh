#!/bin/bash
# C variant seeds 71-100 for stocks17 sweep
...
row = f"{ts},C,{sys.argv[3]},{med:.2f},{p10:.2f},{worst:.2f},{neg},{sort:.2f},{sys.argv[4]}"
print(f"  s{sys.argv[3]}: med={med:.2f}% p10={p10:.2f}% neg={neg}/50 sort={sort:.2f}")
open(sys.argv[2], "a").write(row + "\n")
PY
}

echo "=== C_low_tp seeds 71-100 stocks17 ==="
echo "    train=$TRAIN"

for seed in $(seq 71 100); do
    dir="$CKPT_ROOT/s${seed}"
    if [ -f "$dir/eval_lag2.json" ]; then
        echo "[$(date -u +%FT%TZ)] C s${seed}: already done, skip"
        continue
    fi
    mkdir -p "$dir"
    echo "[$(date -u +%FT%TZ)] C_low_tp seed $seed"
