...
def place_limit_buy(
    ...
    qty = quantize_qty(qty, rules.step_size or 0.00001)
    price = quantize_price(price, rules.tick_size or 0.01, side="BUY")
...
def place_limit_sell(
    ...
    rules = resolve_symbol_rules(sym_cfg.binance_pair)
    qty = quantize_qty(qty, rules.step_size or 0.00001)
    price = quantize_price(price, rules.tick_size or 0.01, side="SELL")
    ...
    for sym_name in symbols:
        ...
        if plan.direction == "long" and plan.confidence >= 0.4:
            ...
            if position_pct >= sym_cfg.max_position_pct:
                logger.info(f"  Skip: already at max position ({position_pct:.1%})")
                # But place a take-profit sell if we don't have one
                if plan.sell_price > current_price and position_qty > 0:
                    order = place_limit_sell(sym_cfg, plan.sell_price, position_qty, dry_run)
                    if order:
            ...
            trade_size = min(trade_size, state.available_quote(sym_cfg.quote_asset) * 0.95)
            trade_size = max(trade_size, 0)

            if trade_size < MIN_TRADE_USD:
                logger.info(f"  Skip: trade too small ({trade_size:.2f})")
                continue

            # Ensure we have the right stablecoin
            if not ensure_quote_balance(sym_cfg.quote_asset, trade_size, state, dry_run):
                logger.warning(f"  Skip: insufficient {sym_cfg.quote_asset}")
                continue
            ...
            # Place take-profit sell if we'll have a position
            if plan.sell_price > buy_price and position_qty > 0:
                sell_order = place_limit_sell(sym_cfg, plan.sell_price, position_qty, dry_run)
                if sell_order:
                    orders_placed.append(sell_order)

        elif plan.direction == "hold":
            # If we have a position, consider placing a take-profit
            if position_qty > 0 and plan.sell_price > current_price:
                order = place_limit_sell(sym_cfg, plan.sell_price, position_qty, dry_run)
                if order:
                    orders_placed.append(order)
            else:
                logger.info(f"  Holding (no action)")
