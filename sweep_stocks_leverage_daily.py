#!/usr/bin/env python3
"""Stocks 2x leverage sweep for daily RL trading.

Trains on stocks15 daily data with leverage (1x, 2x) x trade_penalty variations
x seeds, then evaluates via C-env and market sim.

Usage:
    python -u sweep_stocks_leverage_daily.py
    python -u sweep_stocks_leverage_daily.py --time-budget 30 --max-configs 2  # smoke test
"""
...
TRAIN_DATA = "pufferlib_market/data/stocks15_daily_train.bin"
VAL_DATA = "pufferlib_market/data/stocks15_daily_val.bin"
CHECKPOINT_ROOT = "pufferlib_market/checkpoints/stocks_leverage"
LEADERBOARD_CSV = "pufferlib_market/stocks_leverage_leaderboard.csv"
...
def build_configs() -> list[SweepConfig]:
    """leverage (1.0, 2.0) x trade_penalty (0.05, 0.10, 0.15, 0.20) x seeds (42, 314) = 16 runs."""
    configs = []
    for lev in [1.0, 2.0]:
...
def main():
    parser = argparse.ArgumentParser(description="Stocks 2x leverage daily RL sweep")
    ...
    if not train_path.exists():
        ...
        print("Run the stocks15 daily data export (Unit 1) first.")
        sys.exit(1)
    ...
    print(f"Stocks leverage daily sweep: {len(configs)} configs")
    print(f"  Train data: {train_path}")
    print(f"  Val data:   {val_path}")
    print(f"  Time budget: {args.time_budget}s per config")
    print(f"  Leverages: 1x, 2x | Trade penalties: 0.05, 0.10, 0.15, 0.20")
    print(f"  Seeds: 42, 314 | periods_per_year={PERIODS_PER_YEAR}")
    print(f"  stocks long-only (--disable-shorts), fee={FEE_RATE}, borrow_apr={SHORT_BORROW_APR}")
    ...
    if not args.skip_market_sim and results_for_sim:
        ...
        if sim_results:
            ...
            sim_json_path = REPO / "pufferlib_market" / "stocks_leverage_market_sim.json"
