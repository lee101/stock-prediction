...
def _compute_fee_changes(
    positions: np.ndarray, fee: float, turnover_proportional: bool = True,
) -> np.ndarray:
    """Per-step cost series.

    With ``turnover_proportional=True`` (the realistic default) the cost
    is ``fee * |Δposition|`` — i.e. trading half a unit costs half the
    fee, so smooth sizing is not over-penalised. The legacy path
    (``turnover_proportional=False``) charges the full flat fee whenever
    the position moves at all, kept for backward compatibility.
    """
    pos_change = np.diff(np.concatenate(([0.0], positions)))
    if turnover_proportional:
        return np.abs(pos_change) * float(fee)
    return (np.abs(pos_change) > 1e-9).astype(float) * float(fee)
...
def run_backtest(
    ...
    fees = _compute_fee_changes(positions, fee, turnover_proportional=turnover_proportional_fee)
...
def grid_search_sizing(
    ...
    for s in scales:
        for c in caps:
            res = run_backtest(
                y_true, y_pred, is_crypto=is_crypto, fee=fee, scale=s, cap=c,
                turnover_proportional_fee=turnover_proportional_fee,
            )
            key = res.total_return
            if best is None or key > best[0]:
                best = (key, res.sharpe, res)
    return best[2] if best else run_backtest(
        y_true, y_pred, is_crypto=is_crypto, fee=fee,
        turnover_proportional_fee=turnover_proportional_fee,
    )
