...
import sharpnessadjustedproximalpolicy2.trainer as trainer_mod
from sharpnessadjustedproximalpolicy2.config import SAPConfig
from sharpnessadjustedproximalpolicy2.trainer import SAPTrainer
...
def _make_tc(**kwargs):
    defaults = dict(
        ...
        checkpoint_root="sharpnessadjustedproximalpolicy2/test_checkpoints",
    )
...
def test_checkpoint_has_sharpness(tmp_path):
    ...
    assert "step_scale" in ckpt["sharpness"]
    assert "source_raw" in ckpt["sharpness"]
...
def test_train_enables_dynamo_suppress_errors(tmp_path, monkeypatch):
    ...
    dm = SyntheticDataModule(n_samples=240, seq_len=48, n_features=22)
    trainer = SAPTrainer(tc, sc, dm)
    trainer.train()
...
def test_grad_noise_injector_is_used(tmp_path, monkeypatch):
    calls = {"count": 0}

    def fake_inject(self, model, step, sharpness_ema):
        calls["count"] += 1

    monkeypatch.setattr(trainer_mod.GradientNoiseInjector, "inject", fake_inject)
