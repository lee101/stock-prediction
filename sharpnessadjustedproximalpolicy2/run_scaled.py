...
Local:
    python -m sharpnessadjustedproximalpolicy2.run_scaled --local --epochs 15
    python -m sharpnessadjustedproximalpolicy2.run_scaled --local --symbols DOGEUSD BTCUSD

Remote (RunPod H100):
    python -m sharpnessadjustedproximalpolicy2.run_scaled --gpu h100 --epochs 15
...
def run_local(args):
    ...
    out_json = REPO / f"sharpnessadjustedproximalpolicy2/scaled_results_{timestamp}.json"
    out_csv = REPO / f"sharpnessadjustedproximalpolicy2/scaled_leaderboard_{timestamp}.csv"
    ...
    for symbol in symbols:
        ...
        for config in configs:
            ...
            result = {
                ...
                "final_step_scale": history[-1].step_scale,
...
def _bootstrap_remote(host: str, port: int, remote_dir: str):
    ...
    excludes = " ".join(f"--exclude={p}" for p in [
        ...
        "sharpnessadjustedproximalpolicy2/checkpoints/",
    ])
    os.system(
    ...
    _ssh(host, port, f"cd {remote_dir} && uv pip install -e . 2>&1 | tail -3")
...
def _run_remote_sweep(host: str, port: int, remote_dir: str, args):
    ...
    cmd_parts = [
        ...
        f"PYTHONUNBUFFERED=1 python -m sharpnessadjustedproximalpolicy2.run_scaled --local --epochs {args.epochs}",
    ]
    if args.compile:
        cmd_parts[-1] += " --compile"
    if args.symbols:
...
def _download_results(host: str, port: int, remote_dir: str):
    ...
    os.system(
        f'rsync -az '
        f'-e "ssh -o StrictHostKeyChecking=no -p {port}" '
        f'root@{host}:{remote_dir}/sharpnessadjustedproximalpolicy2/scaled_*.json '
        f'root@{host}:{remote_dir}/sharpnessadjustedproximalpolicy2/scaled_*.csv '
        f'sharpnessadjustedproximalpolicy2/ 2>/dev/null'
    )
    os.system(
        f'rsync -az '
        f'-e "ssh -o StrictHostKeyChecking=no -p {port}" '
        f'root@{host}:{remote_dir}/sharpnessadjustedproximalpolicy2/checkpoints/ '
        f'sharpnessadjustedproximalpolicy2/checkpoints/ 2>/dev/null'
    )
...
def _save_results(results: list[dict], json_path: Path, csv_path: Path):
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["symbol", "config", "best_epoch", "val_sortino", "val_return", "sharpness", "step_scale", "wall_s", "error"])
        for r in sorted(results, key=lambda x: x.get("best_val_sortino", -999), reverse=True):
            if r.get("error"):
                ...
                w.writerow([
                    ...
                    f"{r.get('final_step_scale', r.get('final_lr_scale', 1)):.2f}",
