...
class SAPHistoryEntry:
    ...
    source_sharpness_raw: float = 0.0
...
class SAPTrainer:
    ...
    def train(self) -> tuple[TrainingArtifacts, list[SAPHistoryEntry]]:
        ...
        for epoch in range(1, self.tc.epochs + 1):
            ...
            entry = SAPHistoryEntry(
                ...
                source_sharpness_raw=sharp_state.source_raw,
                loss_delta=sharp_state.loss_delta,
                step_scale=sharp_state.step_scale,
            ...
            print(
                ...
                f"Sharp {sharp_state.ema:.3f} Step {sharp_state.step_scale:.2f} | "
                f"{wall_time:.1f}s",
                flush=True,
            )
        ...
        if swa and swa.n_averaged > 1:
            ...
            swa_entry = SAPHistoryEntry(
                ...
                sharpness_raw=0, sharpness_ema=0, source_sharpness_raw=0, loss_delta=0, step_scale=1, wall_time_s=0,
            )
    ...
    def _train_epoch(
        ...
        for batch_idx, batch in enumerate(loader):
            if self.sc.max_train_batches and steps >= self.sc.max_train_batches:
                break
            ...
            if self.sc.use_adaptive_feature_noise and sharp_state.ema > 0:
                target_sharp = max(getattr(sam_opt, "target_sharpness", self.sc.target_sharpness), 1e-8)
                ratio = min(sharp_state.ema / target_sharp, 3.0)
            ...
            if isinstance(sam_opt, FullSAMOptimizer):
                ...
                _, _ = sam_opt.step_with_sam(forward_and_loss, base_loss=loss.item())
            ...
            elif isinstance(sam_opt, DirectionalSharpnessProximalOptimizer):
                sam_opt.zero_grad(set_to_none=True)
                loss = forward_and_loss()
                loss.backward()
                if grad_noise:
                ...
                self._apply_lr_schedule(base_opt, global_step)
                sam_opt.update_base_lrs()
                sam_opt.step(loss_fn=forward_and_loss, base_loss=loss.item())
            else:
                # Baseline: no SAM
    ...
    def _val_epoch(self, model: BinancePolicyBase, loader, epoch: int) -> dict[str, float]:
        ...
        with torch.inference_mode():
            for batch in loader:
                if self.sc.max_val_batches and steps >= self.sc.max_val_batches:
                    break
                features = batch["features"].to(dev, non_blocking=nb)
                highs = batch["high"].to(dev, non_blocking=nb)
                lows = batch["low"].to(dev, non_blocking=nb)
    ...
    def _wrap_with_sam(self, base_opt):
        ...
        if mode in {"periodic", "proximal", "directional_proximal"}:
            return DirectionalSharpnessProximalOptimizer(
                base_opt,
                rho=self.sc.rho,
                probe_every=self.sc.probe_every,
                ema_beta=self.sc.ema_beta,
                target_sharpness=self.sc.target_sharpness,
                min_scale=self.sc.min_step_scale,
                max_scale=self.sc.max_step_scale,
                warmup_probes=self.sc.warmup_probes,
                scale_beta=self.sc.step_scale_beta,
                flat_bonus=self.sc.flat_bonus,
                sharp_penalty=self.sc.sharp_penalty,
                loss_penalty=self.sc.loss_penalty,
            )
        elif mode == "full_sam":
    ...
    def _save_checkpoint(self, model, epoch, metrics, sharp_state) -> Path:
        ...
        payload = {
            ...
            "sharpness": {
                "raw": sharp_state.raw,
                "ema": sharp_state.ema,
                "source_raw": sharp_state.source_raw,
                "loss_delta": sharp_state.loss_delta,
                "step_scale": sharp_state.step_scale,
        ...
        data = [
            {
                ...
                "source_sharpness_raw": h.source_sharpness_raw, "loss_delta": h.loss_delta,
                "step_scale": h.step_scale, "wall_time_s": h.wall_time_s,
            }
            for h in history
        ]
