"""
Test comparing Claude Opus 4.6 (with extended thinking) vs Chronos2 for time series forecasting.
Measures MAE for return forecasting on BTCUSD data.

This intentionally mirrors `test_opus45_vs_chronos2.py` so the only meaningful change is the model ID.
"""
...
# Initialize Anthropic client for Opus 4.6
...
async def query_opus46_async(prompt: str, system_message: str, model_id: str = OPUS46_MODEL_ID) -> Optional[str]:
    """Query Claude Opus 4.6 with extended thinking for predictions with caching."""
    try:
        ...
        logger.error(f"Error in Opus 4.6 query: {e}")
        return None
...
opus46_abs_error_sum = 0.0
prediction_count = 0

print("Generating forecasts with Opus 4.6 vs Chronos2...")
print(f"Opus model ID: {OPUS46_MODEL_ID}")
...
with tqdm(range(start_idx, end_idx), desc="Forecasting") as progress_bar:
    for t in progress_bar:
        ...
        opus46_pred = analyse_prediction(opus46_response)
        ...
        opus46_forecasts.append({
            'date': data.index[t],
            'actual': actual,
            'predicted': opus46_pred
        })

        prediction_count += 1
        chronos2_abs_error_sum += abs(actual - chronos2_pred)
        opus46_abs_error_sum += abs(actual - opus46_pred)

        progress_bar.set_postfix(
            chronos2_mae=chronos2_abs_error_sum / prediction_count,
            opus46_mae=opus46_abs_error_sum / prediction_count,
        )

chronos2_df = pd.DataFrame(chronos2_forecasts)
opus46_df = pd.DataFrame(opus46_forecasts)
...
opus46_mape = mean_absolute_percentage_error(opus46_df['actual'], opus46_df['predicted'])
opus46_mae = mean_absolute_error(opus46_df['actual'], opus46_df['predicted'])

print("\n" + "=" * 60)
print("RESULTS: Opus 4.6 vs Chronos2 Forecasting Comparison")
print("=" * 60)
print(f"\nChronos2 MAPE: {chronos2_mape:.6f}")
print(f"Chronos2 MAE: {chronos2_mae:.6f}")
print(f"\nOpus 4.6 MAPE: {opus46_mape:.6f}")
print(f"Opus 4.6 MAE: {opus46_mae:.6f}")

# Determine winner
print("\n" + "-" * 60)
if chronos2_mae < opus46_mae:
    winner = "Chronos2"
    improvement = ((opus46_mae - chronos2_mae) / opus46_mae) * 100
    print(f"WINNER (lowest MAE): {winner}")
    print(f"Chronos2 MAE is {improvement:.2f}% better than Opus 4.6")
else:
    winner = "Opus 4.6"
    improvement = ((chronos2_mae - opus46_mae) / chronos2_mae) * 100
    print(f"WINNER (lowest MAE): {winner}")
    print(f"Opus 4.6 MAE is {improvement:.2f}% better than Chronos2")
print("-" * 60)
...
plt.plot(opus46_df.index, opus46_df['predicted'], label='Opus 4.6 Predicted Returns', color='green', linestyle='--')
plt.title('Return Predictions: Opus 4.6 vs Chronos2 on BTCUSD')
plt.legend()
plt.tight_layout()
