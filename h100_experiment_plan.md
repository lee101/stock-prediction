...
## Executive Summary — v7 (FINAL RECOMMENDED H100 CONFIG)
...
- Generated by  using yfinance with price-continuity scaling at junction
- Command:  (only needs re-run if data updates)

### 3. H100 final command (v7)
```bash
...
# Alternative: stocks11 extended (2x data, unknown hit rate locally due to GPU contention)
python launch_stocks_autoresearch_remote.py --gpu-type h100 --max-trials 500 \
