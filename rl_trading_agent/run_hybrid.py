"""
RL + LLM Hybrid Trading Agent.

Pipeline:
1. Load trained PPO model (crypto12, 300M steps, 2659x returns)
2. For each hour in the backtest window:
   ...
   c. Build a prompt showing:
      - Current market data (last 24h OHLCV)
      - Chronos2 forecasts (1h, 24h ahead)
      - RL model's current plan (direction, symbol, confidence)
      - Previous hour's RL plan and what happened
   d. Ask DeepSeek Reasoner to think through and improve the plan
   e. Use the LLM's refined plan for trading
3. Run market simulation on the refined plans

Usage:
  python -m rl_trading_agent.run_hybrid --days 7 --parallel 3
"""
...
class MLPPolicy(nn.Module):
    """MLP policy matching crypto12 checkpoint architecture."""
...
# Symbols the RL model was trained on (crypto12)
CRYPTO12_SYMBOLS = [
    "BTCUSD", "ETHUSD", "SOLUSD", "LINKUSD", "UNIUSD",
    "AAVEUSD", "AVAXUSD", "DOTUSD", "DOGEUSD", "LTCUSD",
    "ALGOUSD", "XRPUSD",
]

# We backtest on the 5 that have good forecast coverage
BACKTEST_SYMBOLS = ["BTCUSD", "ETHUSD", "SOLUSD", "LINKUSD", "UNIUSD"]

CHECKPOINT = REPO / "pufferlib_market" / "checkpoints" / "crypto12_ppo_v8_h1024_300M_annealLR" / "best.pt"


def compute_features_from_bars(bars_df: pd.DataFrame, sym_idx: int, num_symbols: int,
                                fc_h1: dict | None, fc_h24: dict | None,
                                close: float) -> np.ndarray:
    """Compute the 16 features for one symbol at one timestep matching MKTD format."""
    feats = np.zeros(16, dtype=np.float32)

    closes = bars_df["close"].values[-72:] if len(bars_df) >= 72 else bars_df["close"].values
    highs = bars_df["high"].values[-72:] if len(bars_df) >= 72 else bars_df["high"].values
    lows = bars_df["low"].values[-72:] if len(bars_df) >= 72 else bars_df["low"].values

    # Chronos forecast deltas
    if fc_h1 is not None:
        feats[0] = (fc_h1.get("predicted_close_p50", close) - close) / max(close, 1e-8)
        feats[1] = (fc_h1.get("predicted_high_p50", close) - close) / max(close, 1e-8)
        feats[2] = (fc_h1.get("predicted_low_p50", close) - close) / max(close, 1e-8)
        feats[6] = 0.8  # h1 confidence proxy
    if fc_h24 is not None:
        feats[3] = (fc_h24.get("predicted_close_p50", close) - close) / max(close, 1e-8)
        feats[4] = (fc_h24.get("predicted_high_p50", close) - close) / max(close, 1e-8)
        feats[5] = (fc_h24.get("predicted_low_p50", close) - close) / max(close, 1e-8)
        feats[7] = 0.7  # h24 confidence proxy

    # Price-based features
    if len(closes) >= 2:
        feats[8] = (closes[-1] - closes[-2]) / max(closes[-2], 1e-8)  # return_1h
    if len(closes) >= 25:
        feats[9] = (closes[-1] - closes[-25]) / max(closes[-25], 1e-8)  # return_24h

    if len(closes) >= 24:
        feats[10] = np.std(closes[-24:]) / max(np.mean(closes[-24:]), 1e-8)  # vol_24h
        ma24 = np.mean(closes[-24:])
        feats[11] = (close - ma24) / max(ma24, 1e-8)  # ma_delta_24h
    if len(closes) >= 72:
        ma72 = np.mean(closes[-72:])
        feats[12] = (close - ma72) / max(ma72, 1e-8)  # ma_delta_72h
        feats[14] = (closes[-1] - closes[-72]) / max(closes[-72], 1e-8)  # trend_72h

    if len(highs) >= 24 and len(lows) >= 24:
        atr = np.mean(highs[-24:] - lows[-24:])
        feats[13] = atr / max(close, 1e-8)  # atr_pct_24h

    if len(closes) >= 72:
        peak = np.max(closes[-72:])
        feats[15] = (peak - close) / max(peak, 1e-8)  # drawdown_72h
...
def _compute_trend_context(history_rows: list[dict]) -> dict:
    ...
    # Volatility (24h ATR as % of price)
    if len(highs) >= 24:
    ...
    # Trend direction: count of up vs down hours in last 12
    if len(closes) >= 13:
...
def build_hybrid_prompt(
    ...
    # Format forecasts with directional signal
    fc_text = ""
    fc_bullish = 0  # count bullish forecast signals
    if fc_1h:
    ...
    # Format RL signal with interpretation
    ...
    if rl_signal.symbol:
        rl_text += f", Allocation: {rl_signal.allocation_pct:.0%}"
...
SYMBOL: {symbol} (long only, spot market, 0.08% maker fee)
...
RL MODEL (trained 300M steps, 2659x backtested returns):
  {rl_text}
  NOTE: The RL model frequently signals short on this data. Its short signals should be treated as general caution, not a hard constraint — we can only go long on spot.
...
def run_hybrid_backtest(
    ...
    cache_model = f"rl-hybrid-v7-{model}"

    print(f"\n{'='*70}")
    print(f"RL + LLM Hybrid Trading Agent")
    print(f"RL checkpoint: {CHECKPOINT.name}")
    print(f"LLM: {model}")
    print(f"Symbols: {symbols}")
    print(f"Days: {days} | Parallel: {parallel}")
    print(f"{'='*70}\n")

    # Load RL model directly (MLP architecture)
    print("Loading RL model...")
    ckpt = torch.load(str(CHECKPOINT), map_location="cpu", weights_only=False)
    state_dict = ckpt["model"]
    num_symbols = len(CRYPTO12_SYMBOLS)
    obs_size = num_symbols * 16 + 5 + num_symbols  # 197 for crypto12
    # Infer hidden from encoder weights
    ...
    def get_rl_signal(features_all, prices_dict):
        ...
        obs[num_symbols * 16] = 1.0  # cash/10000 (normalized)
        ...
        sym = CRYPTO12_SYMBOLS[action_idx] if action_idx < num_symbols else "?"
        direction = "short" if is_short else "long"
        return TradingSignal(f"{direction}_{sym}", sym, direction, confidence, value_est, 1.0, 0.0)

    # Load market data + compute proper MKTD features for RL model
    DATA_ROOT = REPO / "trainingdatahourly"
    FC_ROOT = REPO / "binanceneural" / "forecast_cache"
    all_bars = {}
    all_fc_h1 = {}
    all_fc_h24 = {}
    all_mktd_features = {}  # {sym: DataFrame with 16 feature columns}
    ...
    for sym in symbols:
        ...
        # Compute MKTD features using the export pipeline
        try:
    ...
    # Load MKTD features for ALL crypto12 symbols (RL model needs all 12 simultaneously)
    all_c12_features = {}
    for c12_sym in CRYPTO12_SYMBOLS:
        try:
            price_df = _read_hourly_prices(c12_sym, DATA_ROOT)
            fc_h1_raw = _read_forecast(c12_sym, FC_ROOT, 1)
            fc_h24_raw = _read_forecast(c12_sym, FC_ROOT, 24)
            feat_df = compute_mktd_features(price_df, fc_h1_raw, fc_h24_raw)
            all_c12_features[c12_sym] = feat_df
        except Exception:
            all_c12_features[c12_sym] = None
    print(f"  Loaded MKTD features for {sum(1 for v in all_c12_features.values() if v is not None)}/{len(CRYPTO12_SYMBOLS)} crypto12 symbols")

    def get_all_features_at(ts):
        """Get feature vector for ALL 12 symbols at timestamp ts."""
        features = np.zeros((len(CRYPTO12_SYMBOLS), 16), dtype=np.float32)
        ts_floor = ts.floor("h") if hasattr(ts, "floor") else pd.Timestamp(ts).floor("h")
        for idx, c12_sym in enumerate(CRYPTO12_SYMBOLS):
            mktd = all_c12_features.get(c12_sym)
            if mktd is None:
                continue
    ...
    # Build all tasks: for each bar, get RL signal + build prompt
    print("\n  Computing RL signals and building prompts...")
    tasks = []  # (sym, bar_dict, prompt, rl_signal)
    prev_signals = {sym: None for sym in symbols}
    prev_outcomes = {sym: "N/A" for sym in symbols}

    for sym in symbols:
        sym_bars = all_bars[sym]
        sym_idx = CRYPTO12_SYMBOLS.index(sym) if sym in CRYPTO12_SYMBOLS else 0
        ...
        for i, (_, bar) in enumerate(window.iterrows()):
            ...
            # Get Chronos forecasts
            fc_1h = get_forecast_at(all_fc_h1[sym], ts)
            fc_24h = get_forecast_at(all_fc_h24[sym], ts)
            close = float(bar["close"])

            # Get proper MKTD features for ALL 12 symbols at this timestamp
            features_all = get_all_features_at(ts)

            # Get RL signal with full 12-symbol context
            rl_signal = get_rl_signal(features_all, {sym: close})

            # Track previous signals for context
            prev_signal = prev_signals[sym]
            prev_outcome = prev_outcomes[sym]

            # Build hybrid prompt (use full 72h for trend computation, prompt shows last 12)
    ...
    if parallel > 1:
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel) as pool:
            futures = {
                pool.submit(_do_call, *tasks[i], i): i
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: RL+LLM Hybrid ({model})")
    print(f"{'='*70}")
    ...
    # Per-week breakdown (for multi-week runs)
    if days > 7 and all_trades:
        print(f"\n  {'─'*50}")
        ...
        for t in all_trades:
            ...
            week_start = (ts - timedelta(days=ts.weekday())).strftime("%m/%d")
            label = f"W{week_key} ({week_start})"
            if label not in week_stats:
                week_stats[label] = {"entries": 0, "exits": 0, "pnl": 0, "fees": 0}
            if t["side"] in ("buy", "short"):
                week_stats[label]["entries"] += 1
            else:
                week_stats[label]["exits"] += 1
            week_stats[label]["pnl"] += t["realized_pnl"]
            week_stats[label]["fees"] += t["fee"]
        for wk, ws in sorted(week_stats.items()):
            net = ws["pnl"] - ws["fees"]
            print(f"  {wk}: {ws['entries']} entries, PnL=${ws['pnl']:+.2f}, fees=${ws['fees']:.2f}, net=${net:+.2f}")
        print(f"  {'─'*50}")

    # LLM entry rate
    total_long = sum(1 for _, _, p, _ in results if p.direction == "long")
    entry_rate = total_long / len(results) * 100 if results else 0
    print(f"\n  LLM entry rate: {total_long}/{len(results)} ({entry_rate:.1f}%)")
    ...
    result_data = {
        "experiment": "rl_llm_hybrid",
        "rl_checkpoint": str(CHECKPOINT.name),
        "llm_model": model,
        "symbols": symbols,
        "days": days,
    ...
    tag = f"rl_hybrid_{model.replace('-', '_')}_{days}d_{'_'.join(symbols)}"
...
def main():
    parser = argparse.ArgumentParser(description="RL + LLM Hybrid Trading Agent")
