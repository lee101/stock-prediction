...
def simulate_strategy(
    ...
    *,
    ...
    expanded_limit: Optional[int] = None,
) -> SimulationReport:
    daily_snapshots: List[DailySnapshot] = []
    ...
    with activate_simulation(
        ...
        base_min_positions = getattr(trade_module, "DEFAULT_MIN_CORE_POSITIONS", 1)
        effective_min_positions = (
            max(1, int(min_positions))
            if min_positions is not None
            else max(1, int(base_min_positions))
        )
        base_expanded = getattr(trade_module, "EXPANDED_PORTFOLIO", effective_min_positions)
        position_cap = max(int(top_k), effective_min_positions)
        if expanded_limit is None:
            effective_expanded = max(base_expanded, position_cap)
        else:
            effective_expanded = max(position_cap, max(1, int(expanded_limit)))
        ...
        for day in range(days):
            ...
            portfolio = trade_module.build_portfolio(
                ...
                max_expanded=effective_expanded,
            )
