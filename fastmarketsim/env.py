...
def _coerce_filter_timestamp(index: "pd.DatetimeIndex", raw_value: str | None):
    ...
    def _read_csv(self, path: Path):
        ...
        if (start is not None or end is not None) and isinstance(frame.index, pd.DatetimeIndex):
            if start is not None:
                frame = frame[frame.index >= _coerce_filter_timestamp(frame.index, start)]
            if end is not None:
                frame = frame[frame.index <= _coerce_filter_timestamp(frame.index, end)]
        return frame
