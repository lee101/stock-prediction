#!/usr/bin/env python3
"""Btcmarketsbot-style training using existing infrastructure with simpler config."""
...
def train_bitbank_style(
    ...
    # Btcmarketsbot-style: simpler model (4 layers, 256 dim) vs nano (6 layers, 512 dim)
    config = TrainingConfig(
        ...
        learning_rate=3e-4,
        weight_decay=1e-4,
        optimizer_name="adamw",  # Simpler optimizer like btcmarketsbot
        model_arch="classic",  # Use classic transformer instead of nano
...
def main():
    ...
    run_name = args.run_name or f"bitbank_h{args.hidden_dim}_l{args.n_layers}_{run_id}"

    logger.info(f"Training btcmarketsbot-style: {args.epochs} epochs, hidden={args.hidden_dim}, layers={args.n_layers}")
    ckpt, test_frame, history = train_bitbank_style(
        args.epochs, run_name, args.hidden_dim, args.n_layers
    )
    ...
    print(f"\n{'='*60}")
    print(f"Btcmarketsbot-Style Result ({args.epochs} epochs, 10bp fee)")
