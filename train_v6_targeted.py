#!/usr/bin/env python3
"""V6 Targeted: Train on specific target symbols only.

Like V4.2 but with gradient accumulation and cosine LR.

Usage:
    python train_v6_targeted.py --epochs 3
"""
...
# Target symbols only (best performers, exclude META/MSFT)
        ...
        for batch in dataloader:
            ...
            with autocast(device_type=self.device.type, dtype=self.amp_dtype, enabled=self.use_amp):
                ...
                loss = (
                    ...
                    - result.sharpe * 0.5
                    + result.forced_exit_rate * 2.0  # V4.2-style penalty
...
def main():
    ...
    if args.run_name is None:
        args.run_name = f"v6_targeted_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

    logger.info(f"V6 TARGETED on {len(TARGET_SYMBOLS)} symbols: {TARGET_SYMBOLS}")
    ...
    trainer = V6TargetedTrainer(training_config, data_module, grad_accum_steps=args.grad_accum)
