...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Grid-sweep FastForecaster2 objective hyperparameters.")
    parser.add_argument("--dataset", choices=["hourly", "daily", "custom"], default="hourly")
    parser.add_argument("--data-dir", type=Path, default=None)
    parser.add_argument("--output-dir", type=Path, default=Path("fastforecaster2") / "sweeps")
...
def build_base_config(args: argparse.Namespace, sweep_output: Path) -> FastForecaster2Config:
...
def _trial_name(index: int, config: FastForecaster2Config) -> str:
...
def _trial_config(
    base_cfg: FastForecaster2Config,
    *,
    output_dir: Path,
...
def _logger_context(
    *,
    ...
    return WandBoardLogger(
        ...
        tensorboard_subdir=f"fastforecaster2/sweeps/{run_name}",
        enable_wandb=True,
        log_metrics=True,
...
def main() -> None:
    ...
    print(f"[fastforecaster2-sweep] Running {len(combos)} trials")
    ...
    for trial_idx, (rlw, dlw, dms, lr, wd, dropout, hwp, qk, ema_eval, ema_decay) in enumerate(combos, start=1):
        ...
        print(f"[fastforecaster2-sweep] Trial {trial_idx}/{len(combos)}: {name}")

        try:
            with _logger_context(args=args, run_name=name, cfg=trial_cfg) as metrics_logger:
                trainer = FastForecaster2Trainer(trial_cfg, metrics_logger=metrics_logger)
            ...
            print(
                "[fastforecaster2-sweep] Completed "
            ...
            results.append(result)
            print(f"[fastforecaster2-sweep] Trial failed: {exc}")
    ...
    if ok_results:
        ...
        print(
            "[fastforecaster2-sweep] Best trial: "
            f"{best['trial_name']} val_mae={best['summary']['best_val_mae']:.6f} "
            f"test_mae={best['summary']['test_mae']:.6f}"
        )

    print(f"[fastforecaster2-sweep] Results written to {results_path}")
