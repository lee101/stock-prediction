...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train FastForecaster2 for low validation MAE.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("fastforecaster2") / "artifacts")
    ...
    parser.add_argument("--chronos-embeddings-path", type=Path, default=None)
    parser.add_argument("--chronos-embeddings-blend", type=float, default=0.0)

    parser.add_argument("--market-sim-eval", dest="use_market_sim_eval", action="store_true", default=True)
    parser.add_argument("--no-market-sim-eval", dest="use_market_sim_eval", action="store_false")
    parser.add_argument("--market-sim-fee", type=float, default=0.0008)
    parser.add_argument("--market-sim-initial-cash", type=float, default=10_000.0)
    parser.add_argument("--market-sim-max-hold-hours", type=int, default=None)
    parser.add_argument("--market-sim-buy-threshold", type=float, default=0.001)
    parser.add_argument("--market-sim-sell-threshold", type=float, default=0.001)
    parser.add_argument("--market-sim-entry-score-threshold", type=float, default=0.0)
    parser.add_argument("--market-sim-max-trade-intensity", type=float, default=35.0)
    parser.add_argument("--market-sim-min-trade-intensity", type=float, default=2.0)
    parser.add_argument("--market-sim-top-k", type=int, default=3)
    parser.add_argument("--market-sim-signal-ema-alpha", type=float, default=0.25)
    parser.add_argument("--market-sim-switch-score-gap", type=float, default=0.0)
    parser.add_argument("--market-sim-entry-buffer-bps", type=float, default=1.0)
    parser.add_argument("--market-sim-exit-buffer-bps", type=float, default=1.0)
    parser.add_argument("--market-sim-take-profit-scale", type=float, default=0.65)
    parser.add_argument("--market-sim-stop-loss-scale", type=float, default=0.65)
    parser.add_argument("--market-sim-vol-lookback", type=int, default=32)
    parser.add_argument("--market-sim-vol-target", type=float, default=0.025)
...
def build_config(args: argparse.Namespace) -> FastForecaster2Config:
    ...
    return FastForecaster2Config(
        ...
        seed=args.seed,
        device=args.device,
        chronos_embeddings_path=args.chronos_embeddings_path,
        chronos_embeddings_blend=args.chronos_embeddings_blend,
        use_market_sim_eval=args.use_market_sim_eval,
        market_sim_fee=args.market_sim_fee,
        market_sim_initial_cash=args.market_sim_initial_cash,
        market_sim_max_hold_hours=args.market_sim_max_hold_hours,
        market_sim_buy_threshold=args.market_sim_buy_threshold,
        market_sim_sell_threshold=args.market_sim_sell_threshold,
        market_sim_entry_score_threshold=args.market_sim_entry_score_threshold,
        market_sim_max_trade_intensity=args.market_sim_max_trade_intensity,
        market_sim_min_trade_intensity=args.market_sim_min_trade_intensity,
        market_sim_top_k=args.market_sim_top_k,
        market_sim_signal_ema_alpha=args.market_sim_signal_ema_alpha,
        market_sim_switch_score_gap=args.market_sim_switch_score_gap,
        market_sim_entry_buffer_bps=args.market_sim_entry_buffer_bps,
        market_sim_exit_buffer_bps=args.market_sim_exit_buffer_bps,
        market_sim_take_profit_scale=args.market_sim_take_profit_scale,
        market_sim_stop_loss_scale=args.market_sim_stop_loss_scale,
        market_sim_vol_lookback=args.market_sim_vol_lookback,
        market_sim_vol_target=args.market_sim_vol_target,
...
def main() -> None:
    ...
    if config.wandb_project:
        run_name = config.wandb_run_name or f"fastforecaster2_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
        logger_ctx = WandBoardLogger(
            ...
            tensorboard_subdir=f"fastforecaster2/{run_name}",
            enable_wandb=True,
            log_metrics=True,
            config={"fastforecaster2": config.as_dict()},
        )
    else:
        logger_ctx = nullcontext()

    with logger_ctx as metrics_logger:
        trainer = FastForecaster2Trainer(config, metrics_logger=metrics_logger)
        summary = trainer.train()

        if metrics_logger is not None:
            metrics_logger.log({f"summary/{k}": v for k, v in summary.items()}, step=config.epochs + 5)

    print("\n[fastforecaster2] Done. Summary:")
