...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train + simulate SOLUSDT Chronos2 neural hourly policy")
    ...
    parser.add_argument("--horizons", default="1", help="Comma-separated forecast horizons")
    ...
    parser.add_argument("--model-arch", default="classic", help="classic or nano")
    ...
    run_name = args.run_name or time.strftime("chronos_sol_%Y%m%d_%H%M%S")
    checkpoint_root = Path("binancechronossolexperiment/checkpoints") / run_name
    results_root = Path("binancechronossolexperiment/results") / run_name
    plot_root = Path("binancechronossolexperiment/plots") / run_name
    ...
    if not args.no_plot:
        ...
        saved = save_trade_plot(args.symbol.upper(), bars, actions, test_result["result"].per_symbol[args.symbol.upper()], output_path)
