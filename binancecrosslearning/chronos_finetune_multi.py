...
def main() -> None:
    parser = argparse.ArgumentParser(description="Multi-symbol Chronos2 fine-tuning (Binance cross-learning).")
    parser.add_argument(
        "--symbols",
    ...
    parser.add_argument(
        "--preaug-strategy",
        default=None,
        help="Pre-augmentation strategy (baseline, percent_change, log_returns, etc).",
    )
    ...
    run_name = args.run_name or time.strftime("chronos2_binance_multi_%Y%m%d_%H%M%S")
