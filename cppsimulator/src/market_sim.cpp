...
torch::Tensor MarketSimulator::action_to_target(
  ...
  auto a = torch::clamp(unit_action, -1.0, 1.0);
...
torch::Tensor MarketSimulator::fees_at(const torch::Tensor& dpos,
  ...
  auto fee = mag * fee_rate * price;
  auto slip =
      torch::nn::functional::softplus(mag) * (cfg_.fees.slip_bps * 1e-4) * price;
  return fee + slip;
}
...
torch::Tensor MarketSimulator::session_pnl(
  ...
  case Mode::OpenClose: {
    ...
    return pos_target * session_ret * px_open;
  }
  case Mode::Event: {
    auto std_all =
  ...
  default: {
    ...
    return pos_target * move * px_open;
  }
  }
}
...
std::pair<torch::Tensor, torch::Tensor> MarketSimulator::auto_deleverage_close(
    ...
    const torch::Tensor& is_crypto) const {
  auto px_close = st_.ohlc.index({idx::Slice(), t, 3});
  auto cap = cfg_.fees.overnight_max;
  auto capped = torch::clamp(pos_target, -cap, cap);
  auto delta = capped - pos_target;
  auto cost = fees_at(delta, px_close, is_crypto);
  return {capped, cost};
}
...
StepResult MarketSimulator::step(const torch::Tensor& actions) {
  ...
  auto cost_open = fees_at(dpos_open, px_open, is_crypto);
  auto finance = financing_at_open(pos_target, px_open, is_crypto);
  auto pnl = session_pnl(t, pos_target);

  auto [end_pos, cost_close] =
      auto_deleverage_close(t, pos_target, is_crypto);
